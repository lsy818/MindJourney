...
p1_complete_sha256="$(p1_sha256_file "$p1_complete_file")" || return 1
p1_expected_complete_sha256="${P1_EXPECTED_ENV_COMPLETE_SHA256:-}"
if [[ -n "$p1_expected_complete_sha256" \
      && "$p1_expected_complete_sha256" != "$p1_complete_sha256" ]]; then
  echo "Persistent environment COMPLETE SHA256 differs from the submitted environment." >&2
  return 1
fi
...
for p1_expected_version in \
    ...
    "svc.numpy=1.26.0" \
    "svc.numpy-quaternion=2024.0.3"; do
...
if [[ "$(p1_manifest_value "$p1_versions_file" svc.pipeline_import)" != "ok" ]]; then
  echo "Persistent environment does not record a successful SVC pipeline import." >&2
  return 1
fi
...
export P1_ENV_COMPLETE_SHA256="$p1_complete_sha256"
export P1_ENV_ID="mindjourney-p1-persistent-v1-$p1_complete_sha256"
...
unset p1_complete_file p1_complete_sha256 p1_expected_checksum
unset p1_expected_complete_sha256 p1_expected_version
...
unset p1_svc_freeze p1_svc_python p1_versions_file
