...
from stable_virtual_camera.seva.geometry import get_camera_dist, get_plucker_coordinates, to_hom_pose
from stable_virtual_camera.seva.sampling import (
    EulerEDMSampler,
    MultiviewCFG,
    MultiviewTemporalCFG,
    VanillaCFG,
)
from stable_virtual_camera.seva.utils import seed_everything
...
def save_output(
    ...
    for sample in samples:
        ...
        if media_type == "image":
            ...
            iio.imwrite(
                os.path.join(save_path, f"pred.mp4")
                if sample_
...
def run_one_scene(
    ...
    if not use_traj_prior:
        ...
        for i, (
            ...
            if options.get("save_first_pass", False):
                save_output(
                    ...
                    save_path=os.path.join(save_path, f"forward_{i}"),
                    video_save_fps=1,
                )
        ...
        if options.get("save_first_pass", True):
            ...
            video_path_0 = os.path.join(save_path, "pred.mp4")
            yield video_path_0
