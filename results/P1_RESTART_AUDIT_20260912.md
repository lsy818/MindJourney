...
**版本说明：本文记录 r9/1024 的历史检查。用户随后明确要求将最大输出改为8192并保持no-thinking，当前运行已切换独立r10；以 [8192执行清单](./records/P1_OUTPUT8192_EXECUTION_20260912.json)为准。** 原1024是此前Qwen适配加入的默认值，不是论文规定，也不是用户要求。原表把其与用户要求并列的描述不够清楚，现予澄清；其他论文与图片顺序核对结论不变。
...
- 四组首片已于 2026-09-12 13:29–13:30 提交，初始均 PENDING/Priority，尚未进行首题产物核验。[新执行清单](./records/P1_CORRECTED_EXECUTION_20260912.json)记录固定源码及完整参数。已恢复本任务每 10 分钟自动推进（automation ID: `mindjourney-p1`），无实质变化时不重复通知。
