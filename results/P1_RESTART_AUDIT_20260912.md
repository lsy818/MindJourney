...
**版本说明：本文记录 r9/1024 的历史检查。用户随后明确要求将最大输出改为8192并保持no-thinking，当前运行已切换独立r10；以 [8192执行清单](./P1_OUTPUT8192_EXECUTION_20260912.json)为准。** 原1024是此前Qwen适配加入的默认值，不是论文规定，也不是用户要求。原表把其与用户要求并列的描述不够清楚，现予澄清；其他论文与图片顺序核对结论不变。
