...
本文件汇总 4 个数据集 × 4 个模型，共 16 组 SVC 实验。目前 6 组已有可报告结果（其中 SAT-Real 150 / Qwen2.5-VL-72B-Instruct 分列本地 BF16 与 API FP8 两个版本），另 10 组结果待补充或修复重评；“—”表示暂无可报告结果，不代表 0 分。
...
| SAT-Real 150 | Qwen2.5-VL-72B-Instruct（本地 BF16） | P4 | 已完成 | 111 / 150 | 74.00% | [验收汇总](#sat-real-qwen25-vl-72b-local-20260923) |
...
<a id="sat-real-qwen25-vl-72b-local-20260923"></a>

#### 本地 BF16

| 类别 | 正确 / 总数 | 准确率 |
|---|---:|---:|
| `ego_movement`（自身运动） | 19 / 23 | 82.61% |
| `obj_movement`（物体运动） | 19 / 23 | 82.61% |
| `goal_aim`（目标朝向） | 27 / 34 | 79.41% |
| `action_conseq`（动作结果） | 31 / 37 | 83.78% |
| `perspective`（视角判断） | 15 / 33 | 45.45% |
| **总体** | **111 / 150** | **74.00%** |
