...
09-16 01:30最新状态：正确P1仍2/4组全量完成；MMSI72B片0已归档36/100（非全量）。共享datasets容量/inode均报100%，27B片0/1/2因ENOSPC失败，保留56/64/50题检查点；片4运行，其他5个P1排队任务已暂缓以免继续失败。见[存储故障与恢复条件](./records/P1_STORAGE_FULL_20260916_0122.json)。

## 全部实验总览

| 数据集 | 模型 | 优先级 | 状态 | 正确 / 总数 | 准确率 | 结果文件 |
|---|---|---|---|---:|---:|---|
| MMSI-Bench 1000 | Qwen3.5-27B | P1 | 存储阻塞：0/1/2失败保留进度；4运行 | — | — | [续跑清单](./records/P1_AUDITFIX_EXECUTION_20260912.json) |
| MMSI-Bench 1000 | Qwen2.5-VL-72B-Instruct | P1 | 8192片0已归档；片1暂缓等存储恢复 | — | — | [提交记录](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/submission_initial.txt) |
...
旧r10片0–5曾因额外thinking检查失败；r11非致命告警修复保留来源续接7道答案。01:30片0/1/2因共享盘满失败，分别保留56/64/50题；片4仍运行。片3=71779_3及71510片5–7已暂缓，片8–9尚未提交，待存储恢复后按原设置续跑。旧未启动71345_[4–5]经PENDING确认后合并替换，记录全部保留；同组跨数组并发合计≤5。旧1024不合并。见[续跑清单](./records/P1_AUDITFIX_EXECUTION_20260912.json)。

### Qwen2.5-VL-72B-Instruct（P1）

8192版本分片0 `70202_0` 已核验：**36 / 100（36.00%）**，0skip，耗时5:52:39。这是100题分片结果，尚无1000题全量成绩；不将该比例填入全量总览。结果、COMPLETE及552文件完整原始日志均保留，见[核验与归档索引](./records/P1_MMSI72B_SHARD0_20260916.json)。片1已提交为 `72432_1`，现因共享盘满而暂缓，恢复后继续3×H20（TP2+独占SVC1）及显式0.93，其他设置不变；片2–9待提交槽。
