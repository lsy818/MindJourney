...
[合并结果](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/results_merged.json) · [严格验证](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/validation.json) · [逐题来源](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/question_provenance.json) · [最终核验记录](./records/P1_MMSI72B_FINAL_20260921.json)。原协议记录的41次评分、5次最终答案解析错误保持不变（事件次数不等同受影响题数），未重新判分。

第3片旧85题加续跑15题完成。完整新旧日志及预续跑SHA回执保留，但独立85题旧检查点字节副本未找到，不能称已验证保留原始快照；最终100题结果不等于旧85题文件。该限制不影响本次覆盖/计数核验，但不能据此宣称历史字节审计完整或EIO永久修复。

8192输出、65536上下文、BF16、3×H20、TP2+SVC1、0.93显存比例及原SVC设置不变。此结果为paper-aligned SVC multi-image adaptation，非论文原生benchmark；发布代码20步扩散与附录50步差异保留披露。9个既有raw包覆盖10片，CRC、安全成员、结果及传输SHA均通过；常见凭据模式无命中，但不是穷尽秘密审计。用户2026-09-21明确授权发布这组结果，其他组原有发布限制不变。

以下带旧日期的队列、故障和待发布描述仅为历史记录；72B最新状态以上述最终核验为准。
...
09-20 02:12最新状态：正确P1仍2/4组全量完成。27B片0–4已核验，5/8/9运行（保存71/35/16题），6/7排队；72B片0–2已核验，片3因存储I/O错误失败、85题检查点保留，片4自然启动，5/6排队，7–9未交。登录节点数据盘挂载异常、计算节点原数据仍可见；未盲目重试，未停止健康任务。4/5运行、9/10提交。见[故障记录](./records/P1_MMSI72B_SHARD3_IO_20260920.json)。完整raw发布继续等待授权。
...
最终结果：**306 / 1000（30.60%）**；完整合并验证通过，详情及历史检查点审计限制见本页顶部2026-09-21更新。下文为原始运行历史。
...
09-19 04:50该组暂无运行分片；片3/4=`73879_3/4`、片5/6=`73994_5/6`等待调度，片7–9待后续提交槽。各3×H20（TP2＋独占SVC1），实际export显式0.93，跨数组总并发2。原仅排队的73893_5合并入73994，排队年龄重置，旧记录保留；无健康作业取消或重启。[实际提交记录](./submissions/P1_SUBMIT_73994.txt)。
