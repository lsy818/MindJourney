...
MindCube/9B片7=`71002_7`完成，耗时8:40:23。冻结验证器确认95题完整覆盖、30正确/65错误、无重复遗漏、0skip；COMPLETE与结果SHA一致，与既有片run-group相同。8192/no-thinking/BF16/65536及SVC设置不变。95题均由r11新生成，无导入答案。累计完成0–7共8/11片，部分 **316/760（41.58%）**，不是全量成绩。[核验及完整日志索引](./P1_9B_SHARD7_20260914.json)。完整日志已通过95/95原始QA覆盖、结果/COMPLETE逐字节检查、gzip完整性和无凭据扫描，服务器原件全部保留。

最后8/9=`71002_8/9`、10=`71041_10`均运行。片10复用现有节点本地环境，确认SVC生成、评分请求HTTP200和vLLM约87 token/s，不把启动状态冒称已完成首题。跨数组上限4+1=5，实际仅剩3片。已用释放槽补交MMSI/27B片4=`71187_4`，与71064_0、71095_1、71114_2、71151_3合计最大5并发。[实际提交输出](./P1_SUBMIT_71187.txt)。下一次扩片必须将该组所有活动数组合计上限维持≤5，不能再简单增加第6个%1。72B70202_0保持3H20/TP2+独占SVC1/显式.93。
...
MindCube/9B片6=`71002_6`完成，耗时8:03:29。冻结验证器确认95题完整覆盖、37正确/58错误、无重复遗漏、0skip；COMPLETE与结果SHA一致，与既有片run-group完全一致。8192/no-thinking/BF16/65536及SVC设置不变。该片95题均由r11新生成，没有导入答案。累计完成0–6共7/11片，部分 **286/665（43.01%）**，不是全量成绩。[核验及完整日志索引](./P1_9B_SHARD6_20260914.json)。日志包已通过95/95原始QA覆盖、结果/COMPLETE逐字节检查、gzip完整性和无凭据扫描，服务器源文件全部保留。

当前同组7/8/9运行、10排队；71002上限4加71041上限1合计5（实际仅剩4片）。释放的提交槽已补交MMSI/27B片3=`71151_3`，与71064_0、71095_1、71114_2合计最多4并发。[实际提交输出](./P1_SUBMIT_71151.txt)。提交前排除完成、运行和排队的重复片，沿用r11 Run ID/源码与配置。72B70202_0保持TP2+独占SVC1共3H20及显式.93不变。
...
MindCube/9B片5=`71002_5`完成，耗时6:05:57。冻结验证器确认95题完整覆盖、34正确/61错误、无重复遗漏、0skip；COMPLETE与结果SHA一致，与前片run-group完全一致。8192/no-thinking/BF16/65536及SVC设置不变。保留r10导入17题和r11新增78题的来源、原始检查点与完整日志。累计完成0–5共6/11片，部分 **249/570（43.68%）**，不是全量成绩。[核验及日志索引](./P1_9B_SHARD5_20260914.json)。完整日志包已通过95/95原始QA覆盖、结果/COMPLETE/检查点逐字节检查、gzip完整性和无凭据扫描，服务器源文件全部保留。

同组片8已接上运行，当前6/7/8运行、9/10排队；跨数组上限71002%4+71041%1=5保持。释放的提交槽已补交MMSI/27B片2=`71114_2`，与71064_0、71095_1合计最多3并发。[实际提交输出](./P1_SUBMIT_71114.txt)。提交前已排除完成、运行和排队的重复片，沿用r11 Run ID/源码与配置。72B70202_0保持TP2+独占SVC1共3H20及显式.93不变。
...
MindCube/9B片4=`70926_4`完成，耗时8:51:56。冻结验证器确认95题完整覆盖、47正确/48错误、无重复遗漏、0skip、COMPLETE与结果SHA一致；与前四片run-group完全一致，8192/no-thinking/BF16/65536和SVC设置不变。保留r10导入1题及r11新增94题的原始来源。累计完成0–4共5/11片，部分 **215/475（45.26%）**，不代表全量成绩。[核验及完整日志索引](./P1_9B_SHARD4_20260914.json)。日志包已核对95/95原始QA、结果/COMPLETE/原始检查点、gzip完整性及无凭据，服务器所有源文件保留。

将71002数组上限从3调至4，与71041上限1合计5；已完成70926不计入活动并发。5–7运行、8–10排队，每片2×A10080。空出的提交槽用于MMSI/27B片1=`71095_1`，片0仍为71064_0；该组合已有相同设置的r10成功答案，按用户授权的非致命告警版本r11并行续跑，不新增smoke。[实际提交记录](./P1_SUBMIT_71095.txt)。两个数组合计上限2，账户global仍最多5运行/10提交。72B70202_0保持3×H20/TP2+独占SVC1/.93排队。
...
MindCube/Qwen3.8-27B最后片10=`70775_10`完成，48/100，耗时7:40:11。冻结验证器严格合并通过：11片、1050题完整覆盖、无重复遗漏、0 skip；全量 **578/1050（55.05%）**，among=257/600、around=150/250、rotation=171/200。逐题来源明确保留r10的358题及r11的692题，不混入1024结果。[全量核验及日志索引](./P1_MINDCUBE38_FULL_20260914.json)；[分类成绩](./RESULTS.md#qwen38-27bp1)。末片完整日志包964902字节/518文件，哈希、gzip完整性、无凭据扫描、100/100原始QA以及结果/COMPLETE逐字节检查通过；先前各片日志保留。

已实际补交MMSI/Qwen3.5-27B首片=`71064_0`，2×A10080，沿用r11 Run ID、8192/no-thinking/BF16/65536以及固定SVC配置；[实际sbatch提交输出](./P1_SUBMIT_71064.txt)。不重配环境、不增加import/version/smoke，不重启健康作业。MMSI/72B=`70202_0`保持TP2+独占SVC共3×H20、GPU memory utilization=0.93排队。
...
MindCube/9B片3=`70825_3`完成（6:21:35），严格验证35/95、无重复遗漏、0skip，COMPLETE和结果SHA匹配；源码/run-group与既有同组一致，8192/BF16/65536/thinking=false和SVC设置不变。保留r10导入3题和r11新生成92题的明确来源、导入前原始检查点。累计完成0–3共4/11片，部分成绩**168/380（44.21%）**，不是全量成绩。[核验及来源记录](./P1_9B_SHARD3_20260914.json)；[完整原始日志包](./raw_logs/mindcube-qwen35-9b-shard3-raw-20260914.tar.gz)。

已补交最后未提交的9B片10=`71041_10`，每片2×A10080；[实际提交输出](./P1_SUBMIT_71041.txt)。当前9B片4/5运行、6–9排71002、10排71041，活动数组上限1+3+1=5。全部MindCube分片均已完成或提交；下一空余提交槽推进MMSI27B片0。MindCube/3.8最后片10仍运行84/100，9B片4为57/95，片5确认实际推理约87.5 token/s。3运行/6×A100，10/10提交槽满；CPU/GPU资源组合仍不足，其余排队。健康作业不重启，环境和实验设置不变。

## 历史进展：2026-09-14 00:10 五片完整日志补归档完成

之前共享目录读取等待的`70825_3.1`归档步骤已正常完成（23分04秒），没有重启推理。已取回[完整日志包](./raw_logs/mindcube-completed-batch2345-20260913.tar.gz)：5,819,279字节、2,350文件，SHA256为`a3e9699aceb6429c8777fac31fc5ac750916c7830624f2886f96465207ef0fed`。gzip完整性、凭据扫描通过；五片475/475道题原始QA回答全部覆盖，results/COMPLETE及9B导入前检查点与已归档文件逐字节一致，完整pipeline/vLLM日志齐全。原始服务端文件全部保留。[更新后的归档验证记录](./P1_COMPLETED_BATCH_20260913_2345.json)。
...
新增严格验证完成：MindCube/3.8片7=46/95、片8=53/95；MindCube/9B片0=44/95、片1=45/95、片2=44/95。逐片通过冻结r11验证器的完整覆盖/无重复遗漏/0skip检查，结果与COMPLETE哈希及8192/BF16/65536/thinking=false等参数核对通过。9B三片分别保留r10导入4/2/17题的原始检查点和逐题来源，不冒称全部r11新生成。[五片验证与原始结果链接](./P1_COMPLETED_BATCH_20260913_2345.json)。
...
已实际补交9B片5–9为`71002`（%3），与70825/70926各%1合计最大5并发。[实际提交输出](./P1_SUBMIT_71002.txt)。当前3运行/6×A100、10/10提交；新片等待Priority，72B70202_0等待Resources。节点11虽余3张A100，但仅余6CPU、任务需8CPU；其余兼容A100和H20均满，不将空闲GPU误报为可立即启动资源。下一提交槽优先9B片10，然后MMSI27B；不修改健康任务、环境或实验设置。

## 历史进展：2026-09-13 18:07 第8片已核验完成，补交70926

MindCube/3.8片9=`70765_9`完成，49/95，运行5:53:38。严格单片覆盖/无重复遗漏/0skip验证通过，COMPLETE与results哈希匹配，run-group、源码及8192/BF16/65536/thinking=false等参数与既有r11一致。95题均为r11新生成，无导入答案。片0–6及9共8/11片，部分成绩**431/760（56.71%）**，不是全量成绩。[本片核验](./P1_COMPLETED_SHARD9_20260913.json)及[完整原始日志](./raw_logs/mindcube-qwen38-shard9-raw-20260913.tar.gz)已保存，510文件/860171字节，所有服务器原件保留。
...
MindCube/3.8片6=`70438_6`完成，严格验证95题、55正确/40错误、无遗漏重复、0skip；结果与COMPLETE SHA匹配，同组指纹与既有r11一致，8192/BF16/65536/thinking=false及原SVC参数不变。该片95题均由r11新生成，没有导入答案。片0–6累计**382/665（57.44%）**，完成7/11片，仅为部分成绩。[本片核验/归档记录](./P1_COMPLETED_SHARD6_20260913.json)；[完整原始日志包](./raw_logs/mindcube-qwen38-shard6-raw-20260913.tar.gz)包含520文件、873533字节，服务器原件全部保留。
...
重新连通DAAI后确认MindCube/3.8片0–5全部完成且通过严格单片验证：覆盖各95题、无重复遗漏、0skip、8192/BF16/65536/thinking=false、源指纹及COMPLETE SHA一致。旧r10片1/3/4先保留原始结果/COMPLETE，再按明确逐题来源导入r11并重新验证，生成新COMPLETE，不占GPU重跑。r11片0/2/5实际新增56/61/95题，共212题；各抽一题原图→staging→QA顺序一致。[完整核验及来源记录](./P1_AUDITFIX_COMPLETED_20260913_1055.json)。
...
原始results、COMPLETE、导入前检查点及验证记录已逐片归档。[完整日志压缩包](./raw_logs/mindcube-qwen38-shards0-5-raw-20260913.tar.gz)含4,981个文件、7,530,345字节，包括完整attempt日志、原始QA/评分JSON、图序记录及来源；[归档哈希清单](./P1_AUDITFIX_RAW_ARCHIVE_20260913.json)已核对。所有源文件保留，未复制权重/视频或重配环境。单CPU归档复用既有计算节点，不新占GPU。没有更改推理设置或重启健康作业。后续优先完成MindCube/3.8，再利用释放资源推进其他正确P1；排队变化仍使全量ETA不可靠。

## 历史进展：2026-09-13 00:26 修正版首片已开始实际推理

`70438_0`已在hkbugpusrv11获得2×A100，实际attempt为70518。日志确认直接复用`/dev/shm/mj-p1-1194/env-4c3ccc5763b452719ee9564c6afbff63c9e13bfc1f0b6513ddfbfa0b4e5dec50`，没有重新分发环境。SVC生成及两次评分请求已完成，当前进入下一题最终QA，vLLM约28.6 token/s。仍保存39/95，全部为导入旧答案，不能把这个数当成修正版新答题数；还没有新完整题可验证。启动源码指纹为r11，参数记录8192/BF16/65536/thinking=false，原检查点成功接续。[真实启动/生成日志与参数快照](./P1_AUDITFIX_PROGRESS_20260913_0026.json)。
...
独立冻结r11：commit `dac49f6db2f0873134b6905ac2244e4284c09948`，source SHA256 `72626718674f0cdc326e3e80a2e8c97b79b88aa2fad212e9b37cd2a61d6873fa`，目录`/home/comp/24482277/shiyang/MindJourney-p1-output8192-r11-20260912`。不修改r10或重启健康作业。14项相关本地测试通过，没有重装环境/import/version/smoke或扫描模型权重。[新执行清单、实际sbatch与续接来源](./P1_AUDITFIX_EXECUTION_20260912.json)。
...
自动监控已更新：优先保持同组总并发5，健康旧片结束后适当提高70438数组上限；提交槽释放先补本组8–10，再以其他正确P1补位。旧健康片完成先严格验证归档，再带来源导入r11；若失败则续接剩余题，不再用旧致命guard重跑。每片完成立即保存、推送，全部四组完成前不停止。[最新原始运行快照](./P1_AUDITFIX_PROGRESS_20260912_2301.json)。

当前健康片平均每题约4–5分钟，仅粗估它们还需约2.5–3小时；新分片等待资源且修正版尚无实际首题，无法可靠承诺四项P1全量完成时间。

## 历史进展：2026-09-12 21:25分片2失败；21:46轮次连接受阻

21:25核验：MindCube/3.8片2=`70271_2`在3:02:28后FAILED 1:0，保存34/95（答案解析错误2、0skip）。失败栈为最终`answer_scaling`响应触发thinking标签检查，与片0同类，不是本次新增I/O/OOM证据。参数记录仍8192、thinking=false及固定r10源码指纹；错误文案不证明开关未传。见[原始日志尾部与结果指纹](./P1_OUTPUT8192_PROGRESS_20260912_2125.json)。
