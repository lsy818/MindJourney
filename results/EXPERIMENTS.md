...
## 2026-09-21 MMSI/Qwen3.5-27B 最终发布

1000题完整覆盖、去重及逐题来源核验通过：**330正确、670错误、0skip，33.00%**。542题严格schema修复（181正确）＋329题此前未完成的新答案（111正确）＋129题保留旧答案（38正确），三部分精确互斥并覆盖官方1000题。

用户明确要求“两题解析失败就不管了”，因此保留题776的错误判分和题992的正确判分，不再推理或重新评分。两题为旧评分解析失败，不是本次新QA解析失败。129旧答案由126旧干净题与停止前差集623/776/992组成；保留r10继承题33/585/803，其余旧答案为r11。不能声称全量1000题全部新生成、零解析错误或统一schema协议。

用户已明确批准本组结果与完整原始回答/评分日志发布至lsy818/MindJourney，包含其中服务器路径及作业信息。保留14份已验证的新schema原始包，并补充旧r10/r11完整可用JSON、尝试日志和最终来源记录包，共15份；全部在本地回读gzip CRC、成员安全与SHA，1000题QA及871题全部原validator QA/评分API覆盖通过。常见凭据模式扫描无命中，非穷尽秘密审计。服务器原件未修改；不重复已有542/329合并，不需要新GPU推理。

8192输出、65536上下文、BF16/no-thinking、temperature0/top_p1/seed44、2A100/TP1+SVC1及原SVC设置保留。schema输出格式约束与旧解析协议不同，最终明确标记混合协议。官方原图顺序保持；paper-aligned多图适配与20步扩散对论文附录50步的差异继续披露。

[最终核验及原始包索引](./records/P1_MMSI27B_FINAL_20260921.json) · [完整结果与逐题来源](./mmsi/qwen3.5-27b/svc/mj-p1-mmsi-qwen35-27b-mixed-schema8192-final-20260921/)。正确P1四组结果现均已整理归档，本组保留上述两题评分解析失败。其他模型、P2新实验及其他新资料不在本次发布范围。下文旧状态仅供历史追溯。
...
## 历史进展：2026-09-20 02:12 72B片3发生I/O错误（原85题快照保留结论未获独立字节副本验证，见上）

72B片3 `73879_3`（实际allocation74357）于02:02:28失败，退出码1，运行4:13:16；题802读取 `step_0/img_0.png` 报 `Input/output error`，此前同题生成视频亦出现读取告警。**已保存85/100题，无COMPLETE，不记为完成**。不是解析重试或显存不足。原检查点SHA `9c3b7bd92ddaa1a8fd29656678013637c579712d365f34626631d9ab36d310fd`、完整尝试日志及图片原件均保留；未上传原始日志。

登录节点此时未挂载 `/home/datasets`，其路径解析到了 `gpufcluster:/home`；节点16仍正确挂载 `gpufcluster:/datasets`，原目录与结果存在，数据盘9.4T空余。出错图片和视频在节点16两次读回成功，但**根因及持久恢复尚未确认，不能宣称数据丢失或问题已修复**。需要管理员核查登录节点挂载与存储EIO；未执行特权挂载，也未在错误底层路径重建目录。

片4 `73879_4` 已于02:02:54自然启动，已保存1题；27B片5/8/9继续运行，分别已保存71/35/16题，片6/7排队。72B片5/6排队，片7–9未提交。当前4/5运行、9/10提交（含未改动55278），6×A100＋3×H20；27B总并发上限3、72B上限2。本轮无新提交、取消、hold或重启；保留健康推理。待存储路径恢复并完成实时去重/额度/跨数组并发核对后，优先续跑片3剩余题目，不能盲增第三个72B cap1。A100-1小规模试验仍按用户条件停止，不自动恢复。

[故障与检查点记录](./records/P1_MMSI72B_SHARD3_IO_20260920.json)。正确P1仍2/4组全量完成，旧完成片及逐题来源不变；完整raw外部发布仍待明确授权。

## 历史进展：2026-09-19 04:50 27B片3完成核验，片8启动、72B片6补交

MMSI/Qwen3.5-27B片3 `71779_3` 于04:36:55正常完成，**30 / 100（30.00%）**，2×A10080本次分配耗时**2天12:39:09**。严格覆盖100题、无重复遗漏、0skip，COMPLETE与结果SHA一致。1题（803）继承r10、99题来自r11；原始检查点和继承QA均已验证字节一致，逐题来源保留。最终答案解析失败2道（334、156）按原规则计错，评分解析异常150次为事件数，评分协议不变。

[核验与归档记录](./records/P1_MMSI27B_SHARD3_20260919.json) · [结果JSON](./mmsi/qwen3.5-27b/svc/mj-p1-8192-mmsi-qwen35-27b-a100-r11-20260912/question_chunk_3/results.json)。完整原始日志651文件、5,660,066字节，含100题QA、图片顺序、评分、历史尝试及原始检查点；全包读取、成员安全/唯一、原件字节一致与凭据扫描通过。归档使用已有72463计算分配单CPU步骤，服务器原件全部保留。**完整日志包仅保存在服务器/本地；与27B片1、72B片2一样，上传继续等待明确授权。本次只推送结果、核验及调度记录。**

27B片8 `73768_8` 于04:37:01接上节点15的2×A100，04:50已观察到实际生成约28.6 tokens/s，但尚未保存完整答案。片5继续运行，已保存34/100题。没有取消或重启任何运行作业。

04:49:00补交72B片6，并把**仅排队、零运行时间**的旧片5 `73893_5` 合并为 `73994_[5-6%1]`。片5排队年龄重置，旧记录完整保留；73879片3–4的排队年龄不变。72B总并发保持73879cap1＋73994cap1＝**2**；每片**3×H20（TP2＋独占SVC1）**，实际export显式保留 **P1_GPU_MEMORY_UTILIZATION=0.93**。[实际提交输出](./submissions/P1_SUBMIT_73994.txt)。

| 实验 | 已核验完成分片 | 运行分片（作业；已保存题数） | 排队分片（作业） | 当前GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 0：32/100；1：32/100；2：38/100；3：30/100；4：28/100 | 5(`71510_5`；34/100)、8(`73768_8`；0/100，已实际推理) | 6/7(`71510_6/7`)、9(`73846_9`) | 4×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100；1：31/100；2：32/100 | 无 | 3/4(`73879_3/4`)、5/6(`73994_5/6`) | 0 |

正确P1仍**2/4组全量完成**，分片分数不代表1000题全量成绩。账户**2/5运行、10/10提交**（含不修改的55278）；27B跨活动数组总上限3，72B为2。兼容A100节点11/15各仅空1卡、节点12满；H20节点16显示4/8已分配但为MIXED+PLANNED，72B等待Priority调度，不能将剩余卡数视为立即可分配。72B片7–9待后续提交槽。共享盘9.6T可用，旧ENOSPC根因仍未确认。8192/BF16/no-thinking/65536、官方图片顺序和SVC设置不变，Qwen2.5VL不传不支持的thinking参数；未重新配环境或扫描未变权重。排队时间未知，全P1预计完成时间仍无法可靠承诺。

## 历史进展：2026-09-18 21:52 72B片2完成核验，片5补交

MMSI/Qwen2.5-VL-72B片2 `73646_2` 于21:40:03正常完成，**32 / 100（32.00%）**，3×H20耗时**4:34:02**。100题完整覆盖、无重复遗漏、0skip，COMPLETE与结果SHA一致；全部来自冻结r10，无继承答案。最终答案解析失败1道（735）按原规则计错，评分解析异常2次为事件数；原解析器和评分协议不变。

[核验与归档记录](./records/P1_MMSI72B_SHARD2_20260918.json) · [结果JSON](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/question_chunk_2/results.json)。完整原始日志归档516文件、379,392字节，含100题QA和图片顺序；已核验全包、成员安全/唯一、原件字节一致及凭据扫描。**日志包已保存在服务器/本地，外部上传继续等待明确授权；本次只推送结果与记录，不上传原始日志。** 27B片1的原始日志上传也仍待授权。服务器原件全部保留。

21:52:03使用释放的提交槽补交72B片5 `73893_5`，未取消或替换其他排队/运行任务。72B片3–4仍为 `73879_[3-4%1]`，与新73893cap1合计上限**2**。实际export显式保留 **P1_GPU_MEMORY_UTILIZATION=0.93**，保持每片3×H20（TP2＋独占SVC1）、8192/BF16/65536、原RunID/r10和SVC设置。[实际提交输出](./submissions/P1_SUBMIT_73893.txt)。

| 实验 | 已核验完成分片 | 运行分片（作业；保存题数） | 排队分片（作业） | 当前GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 0：32/100；1：32/100；2：38/100；4：28/100 | 3(`71779_3`；92/100)、5(`71510_5`；24/100) | 6/7(`71510_6/7`)、8(`73768_8`)、9(`73846_9`) | 4×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100；1：31/100；2：32/100 | 无 | 3/4(`73879_3/4`)、5(`73893_5`) | 0 |

正确P1仍**2/4组全量完成**，分片分数不代表1000题全量成绩。账户**2/5运行、10/10提交**（含不修改的55278）。H20节点16现8/8卡被其他作业占用，兼容A100节点11/15各仅空1卡、节点12满，当前资源不足以启动排队任务。27B活动数组总上限4，全部分片已提交；72B片6–9尚未提交，等待槽位。共享盘9.6T可用。未重配环境/重复扫描权重，未重启健康任务；官方图片顺序不变，Qwen3 no-thinking不变，Qwen2.5-VL不传不支持的thinking参数。排队时间未知，不承诺全P1准确完成时间。

## 历史进展：2026-09-18 20:01 27B片1完成核验，72B片3–4合并补交

MMSI/Qwen3.5-27B片1 `73395_1` 于19:52:12正常完成，**32 / 100（32.00%）**，2×A10080最后一次续跑耗时**22:20:36**（不含此前失败尝试）。严格覆盖100题、无重复遗漏、0skip，COMPLETE与结果SHA一致。100题均为r11答案，其中64题来自之前保存的r11检查点、36题由本次续跑补齐；无r10继承答案。最终答案解析失败2道（466、139）按原规则计错，评分解析错误127次为事件数；未修改解析器或评分协议。

[核验与归档记录](./records/P1_MMSI27B_SHARD1_20260918.json) · [结果JSON](./mmsi/qwen3.5-27b/svc/mj-p1-8192-mmsi-qwen35-27b-a100-r11-20260912/question_chunk_1/results.json)。630文件、5,078,496字节，100题QA/图片顺序及评分、历史失败尝试日志完整保留；全包读取、成员安全/唯一、原件字节一致与凭据扫描通过。使用已有71779计算分配的单CPU步骤归档，未安装或重新复制环境。服务器原件和旧失败检查点记录全部保留。**完整日志包目前仅保存在服务器和本地，外传上传被自动安全审查拦截，等待用户明确授权；此提交仅发布结果、核验记录及调度状态。** 归档路径及SHA见上述记录，暂不提供尚未上传的GitHub日志链接。

利用片1完成释放的提交槽，19:58:52补交72B片4，并把**仅排队、零运行时间**的旧片3 `73871_3` 合并为新数组 `73879_[3-4%1]`；旧提交记录保留，片3排队年龄因此重置，未触碰任何运行作业。72B总并发为73646cap1＋73879cap1＝**2**。实际sbatch export显式保留 **P1_GPU_MEMORY_UTILIZATION=0.93**，保持TP2＋独占SVC1、每片3×H20、原RunID与r10。[实际提交输出](./submissions/P1_SUBMIT_73879.txt)。

| 实验 | 已归档分片 | 运行分片（作业；保存题数） | 排队分片（作业） | 当前GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 0：32/100；1：32/100；2：38/100；4：28/100 | 3(`71779_3`；85/100)、5(`71510_5`；22/100) | 6/7(`71510_6/7`)、8(`73768_8`)、9(`73846_9`) | 4×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100；1：31/100 | 2(`73646_2`；64/100，实际推理) | 3/4(`73879_3/4`) | 3×H20 |

正确P1仍**2/4组全量完成**；分片分数不是1000题全量成绩。账户**3/5运行、10/10提交**（含不修改的55278）。兼容A100节点11/15和H20节点16各仅空1卡，节点12满，暂不足以启动每片2A100/3H20的作业。27B活动数组合计上限为4；其全部分片已提交，72B片5–9待后续槽位。共享盘9.6T可用，历史ENOSPC根因未确认。8192、BF16、no-thinking、65536、官方图片顺序和SVC不变；Qwen2.5-VL不传不支持的thinking参数。排队及27B长回复使全P1完成时间仍无法可靠承诺。

## 历史进展：2026-09-18 17:13 72B片1完成归档，片2启动、片3补交

MMSI/Qwen2.5-VL-72B片1 `72432_1` 于17:05:44正常完成，**31 / 100（31.00%）**，3×H20耗时**6:11:31**。严格覆盖100题、无重复遗漏、0skip，COMPLETE与结果SHA一致，100题全部来自冻结r10，无继承答案。最终答案解析失败0道，评分解析错误6次为事件数；未修改解析器或评分协议。

[核验与归档记录](./records/P1_MMSI72B_SHARD1_20260918.json) · [结果JSON](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/question_chunk_1/results.json) · [完整原始日志](./raw_logs/mmsi-qwen25vl-72b-shard1-raw-20260918.tar.gz)。576文件、440,315字节，包含100题QA、图片顺序及评分/运行日志；完整读取、成员安全/唯一、原件字节一致与凭据扫描通过。服务器原件全部保留，归档使用已有计算节点单CPU步骤，无环境安装或重新复制。

片2 `73646_2` 于17:06:01自然接上节点16的3×H20，当前模型加载，17:13尚无保存答案；不把RUNNING当作实际答题。17:10:21利用释放提交槽补交片3 `73871_3`，Priority排队。实际sbatch的export显式保留 **P1_GPU_MEMORY_UTILIZATION=0.93**，保持**TP2＋独占SVC1、3×H20、8192、BF16、65536、原RunID与r10**；Qwen2.5-VL不传不支持的thinking参数。旧72432完成后跨72B活动数组为73646cap1＋73871cap1＝**2**。[实际提交输出](./submissions/P1_SUBMIT_73871.txt)。

| 实验 | 已归档分片 | 运行分片（作业；保存题数） | 排队分片（作业） | 当前GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 0：32/100；2：38/100；4：28/100 | 1(`73395_1`；92/100)、3(`71779_3`；80/100)、5(`71510_5`；15/100) | 6/7(`71510_6/7`)、8(`73768_8`)、9(`73846_9`) | 6×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100；1：31/100 | 2(`73646_2`；模型加载) | 3(`73871_3`) | 3×H20 |

正确P1仍**2/4组全量完成**，以上不是1000题全量成绩。账户**4/5运行、10/10提交**（含不修改的55278）；A100节点15仅空1卡，其他兼容节点及H20均满，空余运行名额等待资源。27B全部已提交，72B片4–9待槽。未取消或重启健康任务，官方图片顺序及SVC设置不变。共享盘9.5T可用，旧存储故障根因未确认；单片72B本次约6.2小时，但排队及27B长回复使全P1完成时间仍无法可靠承诺。

## 历史进展：2026-09-18 15:13 27B片2归档，最后未提交的片9已补交

MMSI/Qwen3.5-27B片2 `72732_2` 于14:55:59正常完成，**38 / 100（38.00%）**。严格覆盖100题、无重复遗漏、0skip，COMPLETE与结果SHA一致；100题全部来自r11，之前失败续跑的原始日志/检查点保留。最后一次续跑耗时**27:14:50**，不含更早失败尝试。5道最终答案解析失败按原规则计错；136次评分解析错误是事件数，未改变评分或解析器。

[核验与归档记录](./records/P1_MMSI27B_SHARD2_20260918.json) · [结果JSON](./mmsi/qwen3.5-27b/svc/mj-p1-8192-mmsi-qwen35-27b-a100-r11-20260912/question_chunk_2/results.json) · [完整原始日志](./raw_logs/mmsi-qwen35-27b-shard2-raw-20260918.tar.gz)。571文件、5,119,992字节，包含100题QA、图片顺序、评分及全部尝试日志；全包读取、成员安全/唯一、原件字节一致与凭据扫描通过。归档使用现有计算节点单CPU步骤，未重装/重复制环境，服务器原件不删。

原72732数组已全部终止，不再占活动并发。15:08:14补交27B片9 `73846_9`（2×A100，Priority排队）；27B现无未提交分片。跨数组总上限为71779/71510/73395/73768/73846各1，仍为**5**；72B上限仍2。原RunID、r11/r10冻结源码、8192/BF16/no-thinking/65536、官方图片顺序及SVC设置均不变，72B维持TP2+SVC1、3×H20与原实际export显式0.93；未取消或重启健康任务。[服务器实际提交凭证](./submissions/P1_SUBMIT_73846.txt)。

| 实验 | 已归档分片 | 运行分片（作业；保存题数） | 排队分片（作业） | 当前GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 0：32/100；2：38/100；4：28/100 | 1(`73395_1`；91/100)、3(`71779_3`；79/100)、5(`71510_5`；11/100) | 6/7(`71510_6/7`)、8(`73768_8`)、9(`73846_9`) | 6×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100 | 1(`72432_1`；63/100)，已实际推理 | 2(`73646_2`) | 3×H20 |

正确P1仍**2/4组全量完成**；以上均为分片成绩，不是1000题全量准确率。账户4/5运行、**10/10提交**（含不修改的55278）；已确认兼容节点11/12/15/16 GPU全部已分配，空余运行名额等待集群资源，不等于仍有提交槽。72B片3–9待槽，完成后及时补交并控制所有数组合计并发。共享存储9.6T可用，旧ENOSPC根因仍未确认；全量ETA受排队与逐题耗时影响，暂不能可靠承诺。

## 历史进展：2026-09-18 10:56 72B片1获得H20，运行额度已满

原排队MMSI/Qwen2.5-VL-72B片1 `72432_1` 于10:54:13在节点16获**3×H20**，保持**TP2＋独占SVC1、显式0.93、BF16、8192输出、65536上下文**与冻结r10。Qwen2.5-VL不传不支持的thinking参数。复用已有节点本地环境，日志确认模型加载开始；截至10:56尚未确认新答案或实际QA推理，不能将Slurm RUNNING当成已答题。

账户已达**5/5运行、10/10提交，共8×A100＋3×H20**。27B片1/2/3/5继续运行（保存84/92/75/4题），6/7/8排队；72B片2排队，当前受账户运行上限限制。兼容节点GPU已全部分配，共享存储9.6T可用。没有新增提交、取消或重启健康任务，没有环境重装、重复制或额外权重扫描。27B片9、72B片3–9仍待提交槽位；严格按跨数组合计并发及去重规则补交。

正确P1全量仍2/4组，现有完整/分片成绩不变。[启动与资源记录](./records/P1_MMSI72B_SHARD1_START_20260918.json)。

## 历史进展：2026-09-18 08:46 27B片0完成归档，片5接续运行、片8补交

MMSI/Qwen3.5-27B片0 `72732_0`（实际allocation `73268`）于08:21:49正常完成，**32 / 100（32.00%）**。严格覆盖100题、无重复遗漏、0skip，COMPLETE与结果SHA一致。2题继承r10、98题由r11生成；逐题来源与原始检查点保留。记录2道最终答案解析失败（按原规则计错）和140次评分解析错误事件（不是题数），未修改解析器或评分。

本次续跑作业从09-17 08:18:00至09-18 08:21:49，耗时**24小时03分49秒**，仅为最后一次续跑耗时，不含之前尝试。607文件、5,219,649字节的日志包包含全部100题QA、评分、图片顺序、r10/r11与历次attempt日志；全包读取、成员安全/唯一、原件字节一致、凭据扫描和SHA核验通过。服务器原件不删除。

[核验与归档索引](./records/P1_MMSI27B_SHARD0_20260918.json) · [结果JSON](./mmsi/qwen3.5-27b/svc/mj-p1-8192-mmsi-qwen35-27b-a100-r11-20260912/question_chunk_0/results.json) · [完整原始日志](./raw_logs/mmsi-qwen35-27b-shard0-raw-20260918.tar.gz)。

原排队片5 `71510_5` 于08:22:06接上节点12的2×A100，已观察到实际评分推理；截至08:46保存1题仍为r10继承答案，不能算新完成1题。片1/2/3分别保存81/88/72题。片0完成后先把`72732`总限流2降为1（只剩片2），再利用空出的提交槽于08:28:34补交片8 `73768_8`（2×A100，Priority排队）；跨27B全部数组合计上限仍为**5**，无重复提交。沿用原RunID与冻结r11、8192/BF16/no-thinking/65536和SVC/官方图片顺序。未重启健康任务，未重配或重拷环境，未重复扫描权重。[实际提交输出](./submissions/P1_SUBMIT_73768.txt)。

| 实验 | 已归档分片 | 运行分片（作业） | 排队分片（作业） | GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 0：32/100；4：28/100 | 1(`73395_1`)、2(`72732_2`)、3(`71779_3`)、5(`71510_5`) | 6/7(`71510_6/7`)、8(`73768_8`) | 当前8×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100 | 无 | 1(`72432_1`)、2(`73646_2`) | 每片3×H20，TP2+SVC1，显式0.93 |

正确P1全量仍**2/4组**；上述分片成绩不是1000题全量成绩。账户4/5运行、10/10提交（含不改动的55278）；节点11仅1卡空余，12/15/16全部已分配，暂无满足2/3卡要求的新启动资源。共享存储9.7T可用，历史ENOSPC底层根因未确认。27B片9、72B片3–9待提交槽和合计并发允许时补交。排队/题间耗时不稳定，暂不承诺全量ETA。SSH现有控制连接实测可用，之前本地权限拒绝不代表服务器掉线。

## 历史进展：2026-09-17 21:41 27B片4归档，72B片2已补交

MMSI/Qwen3.5-27B片4 `71510_4` 于21:31:08正常完成：**28 / 100（28.00%）**，耗时2天10:53:07。严格覆盖100题、无重复遗漏、0skip，COMPLETE与结果SHA一致；其中3题来自r10、97题由r11生成，逐题来源独立保存。8道最终答案解析失败按原规则计错，144次评分解析错误是事件数，不能当成题数。此为分片成绩，**不是1000题全量成绩**。

[结果与核验记录](./records/P1_MMSI27B_SHARD4_20260917.json) · [结果JSON](./mmsi/qwen3.5-27b/svc/mj-p1-8192-mmsi-qwen35-27b-a100-r11-20260912/question_chunk_4/results.json) · [完整原始日志](./raw_logs/mmsi-qwen35-27b-shard4-raw-20260917.tar.gz)。完整日志包618文件、5,558,892字节，包含100题QA和图片顺序、评分、r10/r11运行日志和原始检查点；全包读取、成员唯一/安全、原件字节一致和凭据扫描通过。服务器原件不删。归档辅助步骤最初使用旧系统Python退出，随后直接使用已准备节点本地解释器成功，无环境安装/复制，无推理重启。

片1 `73395_1` 于21:31:36在节点11获2×A100，已观察到实际QA/评分推理，暂未见新的ENOSPC；原故障根因仍未确认，不宣称永久修复。21:35:37利用释放的提交槽追加72B片2 `73646_2`，原r10 RunID、**TP2+独占SVC1、3×H20、实际export显式0.93**，8192/BF16/65536及SVC/图片顺序均不变。[实际提交记录](./submissions/P1_SUBMIT_73646.txt)。

| 实验 | 已归档分片 | 运行分片（作业） | 排队分片（作业） | GPU |
|---|---|---|---|---|
| MMSI / Qwen3.5-27B | 4：28/100 | 0(`72732_0`)、1(`73395_1`)、2(`72732_2`)、3(`71779_3`) | 5–7(`71510_5/6/7`) | 当前8×A100 |
| MMSI / Qwen2.5-VL-72B | 0：36/100 | 无 | 1(`72432_1`)、2(`73646_2`) | 每片申请3×H20 |

正确P1全量仍2/4组；MindCube两组已完成不重复。账户4/5运行、10/10提交（含不改动的55278）；27B跨数组总上限5，72B总上限2。选择追加72B片2以填满提交槽，避免给27B再加第六个独立并发名额或取消已有排队任务。节点11仅余1卡，其他兼容节点GPU满，无法再启动2卡/3卡片。共享存储9.8T可用。27B8–9、72B3–9尚未提交；后续根据额度与合计并发补充。排队及题间耗时变化较大，全量ETA暂不可靠。SSH控制连接本轮失效后已重新认证恢复，未改配置、未影响Slurm作业。

## 历史进展：2026-09-17 12:04 片1故障已保留并续交，4片继续运行

27B片1的续跑作业 `72732_1` 于11:40:49再次在写入逐步回答日志时遇到 `ENOSPC`，64/100题检查点哈希未变。共享盘约10T可用，同节点本地磁盘/内存盘有余量；个人配额查询被存储RPC拒绝。对出错题139的step0/step1及片1/2根目录定向临时写入、fsync、回读均成功，但底层原因尚未确认，不能宣称已永久修复。

保留全部原始产物后，11:47已按原r11 Run ID和64题检查点续交一次 `73395_1`（2×A100，排队Priority）。原数组 `72732` 上限从3降到2，仅对应仍运行的0/2片；跨数组上限为 **2+1+1+1=5**。未取消或重启任何健康作业，未改环境、权重、BF16/no-thinking/8192输出/65536上下文/图片顺序或SVC设置。

| 实验 | 运行分片（作业） | 排队分片（作业） | 当前GPU |
|---|---|---|---:|
| MMSI / Qwen3.5-27B | 0(`72732_0`)、2(`72732_2`)、3(`71779_3`)、4(`71510_4`) | 1(`73395_1`)、5–7(`71510_5/6/7`) | 8×A100 |
| MMSI / Qwen2.5-VL-72B | 无；片0已归档36/100 | 1(`72432_1`) | 排队申请3×H20，TP2+SVC1、0.93 |

27B片0/1/2/3/4已保存62/64/50/41/80题；这些是进度，不是全量成绩。正确P1仍2/4组全量完成。账户4/5运行、10/10提交（含不修改的55278）；兼容节点GPU全部已分配，因此本轮不重复补交。27B片8–9、72B片2–9待槽。排队时间及存储风险尚不确定，暂不报承诺性完成时间。

[故障与续交记录](./records/P1_MMSI27B_SHARD1_RETRY_20260917.json) · [实际提交输出](./submissions/P1_SUBMIT_73395.txt)

## 历史进展：2026-09-16 12:51存储可写恢复，释放暂缓任务并续交0–2片

共享盘01:55出现约646G余量，12:43约750G/12278390可用inode；存活27B片4从22题持续推进至40题，近期日志未见新增错误。在现有节点11作业71801内，对片0/1/2/4确切目录各做1MiB临时写入、fsync和回读一致性测试，全部通过并移除测试文件；失败片0/1/2检查点哈希与已归档副本一致。依据跨约10小时的真实输出推进及定向读写证据恢复调度，并非仅凭一次探测。12:51余量约714G，聚合df百分比仍四舍五入显示100%；**存储仍接近满盘，不宣称永久修复**，没有删除实验产物。[恢复核对记录](./records/P1_STORAGE_RECOVERY_20260916_1251.json)。

12:45释放原暂缓`71779_3`、`71510_5/6/7`、`72432_1`，保留原数组和输出目录。确认失败片无活动替代作业后，将27B片0/1/2按原r11 RunID及56/64/50题检查点续交为`72732_[0–2%3]`，每片2×A10080；与71779上限1、71510上限1合计最多5片。原始提交输出确认BF16、thinking=false、65536上下文、8192输出及固定源码/数据/配置哈希均不变，复用已准备环境和资源；72B原提交TP2+独占SVC1、3×H20、显式0.93不变。未取消/重启健康片4。[实际提交输出](./submissions/P1_SUBMIT_72732.txt)。

12:51队列：片4=`71510_4`运行（2×A100，40/100）；27B片0/1/2=`72732_0/1/2`、片3=`71779_3`、片5–7=`71510_5/6/7`及72B片1=`72432_1`排队。账户1/5运行、10/10提交（含不改动的55278）；兼容A100节点11/12/15及H20节点16当时均8/8GPU已分配，剩余运行名额等待实际资源。27B8–9和72B2–9尚未提交。正确P1仍2/4组全量完成；排队及存储风险使全量ETA暂不可靠。若再次ENOSPC/EIO，只暂缓核实尚未启动的P1任务，保留存活推理和全部产物，诊断后再决定续跑。

旧临时维护目录已不存在，Git直连本轮超时；通过已连接GitHub服务读取最新main记录并保存恢复记录，原远端历史、故障证据、170题检查点及完整成绩均保留。

## 历史进展：2026-09-16 01:30共享盘满，3片失败并保护剩余排队任务

27B片0=`71064_0`、片1=`71095_1`分别于01:18:13、01:18:03 FAILED1:0；片2=`71114_2`随后于01:22:07 FAILED1:0。三片均在`_dump_llm_interaction`创建评分回答JSON时遇到`OSError: [Errno28] No space left on device`，不是显存/环境/额外thinking检查失败。检查点仍可读，分别保存56、64、50题，已复制到各Run的`failed_attempts/`独立目录；这170道已保存题不代表完整分片，含原有r10导入答案。所有服务器原始日志、图片、检查点保留。[故障证据、检查点哈希与暂缓记录](./records/P1_STORAGE_FULL_20260916_0122.json)。

登录节点与故障计算节点11独立只读检查均显示`gpufcluster:/datasets`容量129T/129T、可用空间0、inode2150481567已全部占用、IFree0。quota查询到存储服务器的RPC被拒绝，无法确认是否另有个人配额限制；不把聚合满盘归因于本实验单独占用。需要管理员排查共享存储容量/inode并恢复写入，或用户明确指定/授权可用替代存储。未擅自清理或删除结果，未改挂载、模型或论文设置。

01:22逐个确认PENDING后将`71779_3`、`71510_5/6/7`、`72432_1`暂缓，均为JobHeldUser，保留原排队记录，无关55278未动；未取消/重启运行任务。01:30仅`71510_4`仍运行（已保存21题），账户1/5运行、2×A100、7/10提交；暂不为填额度盲目提交到已满存储。旧作业71064/71095/71114已自行失败，并非被主动停止。继续监控存活片和存储状态，存储恢复后先确认实际读写健康，再释放上述5个任务并按检查点恢复失败片0/1/2，保持RunID/r11/8192/BF16/no-thinking/65536及图片顺序/SVC设置。原预计完成时间失效。

## 历史进展：2026-09-16 SSH恢复，72B片0核验归档并补交片1

MMSI/72B片0 `70202_0` 于09-15 17:14:12完成，11:21:33开始，耗时 **5:52:39**，100题中36题正确（36.00%）。冻结r10验证器确认有效题目精确覆盖、无重复遗漏、0skip、运行配置/指纹及输出预算；COMPLETE SHA单独核对一致。这只是**分片成绩，不是全量1000题结果**。[核验与归档索引](./records/P1_MMSI72B_SHARD0_20260916.json)，[分片JSON](./mmsi/qwen2.5-vl-72b/svc/mj-p1-8192-mmsi-qwen25vl-72b-h20-r10-20260912/question_chunk_0/results.json)，[完整原始回答与日志](./raw_logs/mmsi-qwen25vl-72b-shard0-raw-20260916.tar.gz)。552个文件包含100题原始QA和评分/图片顺序/运行日志，计算节点单CPU重叠归档，本地全包读取、成员唯一性、结果字节一致性和凭据扫描通过；服务器原件全部保留。

核对全账户队列和各版本片1不存在后，00:38:49补交 `72432_1`（1%1），保留原Run ID/r10、3×H20（TP2+独占SVC1）、实际sbatch显式`P1_GPU_MEMORY_UTILIZATION=0.93`、8192输出、BF16和65536上下文。Qwen2.5-VL不传不支持的thinking参数，Qwen3关闭thinking；官方图片顺序和SVC设置不变，无新增smoke或环境/权重扫描。[提交记录](./submissions/P1_SUBMIT_72432.txt)。

00:40队列：27B片0/1/2/4=`71064_0/71095_1/71114_2/71510_4`运行（00:24分别保存55/63/49/19题，含已有导入题；运行日志持续更新），片3=`71779_3`、片5–7=`71510`排队；72B片1=`72432_1`排队Priority。账户4/5运行、8×A100、10/10提交槽（含无关55278）；H20节点16为8/8已分配。27B所有活动数组合计上限5，不添加第六个%1；27B8–9、72B2–9尚未提交。优先填额度、其次集中MMSI；没有取消/重启健康作业。72B单片已有约5.9小时参考，队列等待与27B题间耗时波动使全量完成时间无法可靠估计。

用户明确要求重试后，原SSH配置的跳板机及DAAI密码认证均成功，复用连接恢复；断连没有停止Slurm作业。旧本地临时副本缺少Git元数据，保留原目录，新建干净维护副本并按最新远端核对；自动监控已更新路径和作业清单。正确P1仍2/4组全量完成，无新的全量成绩。

## 历史进展：2026-09-15 11:44 72B首题成功，账户运行额度用满

MMSI/72B片0=`70202_0`于11:21:33在节点16启动，复用已准备的节点本地环境。模型服务11:30:21就绪，随后SVC实际生成；11:44已有4道正式保存答案（494、585、33、439），对应原始QA文件存在、无导入检查点。结果记录确认BF16、65536上下文、8192输出、TP2+SVC1及固定r10配置；该模型不传不支持的thinking选项。原提交显式GPU_MEMORY_UTILIZATION=.93保持不变。[首题成功与实时配置证据](./records/P1_MMSI72B_FIRST_SUCCESS_20260915.json)。这是运行成功记录，不是完成片或全量成绩；后续扩片不再等待额外smoke。

MMSI27B片0/1/2/4=`71064_0/71095_1/71114_2/71510_4`均实际推理，分别保存35/38/29/4题（片4含3道导入，已有1道新答案）。片3=`71779_3`等待账户运行上限，71510片5–7等待数组共享上限。当前5/5运行、8×A100+3×H20、10/10提交槽，用满账户额度；不额外提交重复片、不取消任何健康任务、不修改设置。节点15再次因无关71859的batch退出失败被DRAIN，不修改管理员状态。正确P1仍2/4组全量完成，继续集中MMSI并在空槽后补齐剩余分片；每片完成即核验归档。

## 历史进展：2026-09-15 10:19 MMSI27B片3管理员取消后恢复排队

`71151_3`于09:20:25在节点15启动，09:24:37结束，Slurm明确记录`CANCELLED by 0`（管理员UID0），运行4分12秒；batch收到SIGTERM，退出15。日志只显示复用已准备好的本地依赖和固定数据/源码记录，没有模型报错；管理员取消的具体原因未知，不把它归因于I/O或thinking。节点15现已解除DRAIN，但8/8GPU均已分配。

核对全账户队列无替代片、片3无COMPLETE且仍只有1道r10导入答案后，使用原r11 Run ID/源码/配置和`--resume`正常补交为`71779_3`（3%1，2×A10080），当前PENDING Priority，无前片完成依赖。旧作业、检查点及日志全部保留，未取消任何健康作业或改变管理员调度状态。[实际提交记录](./submissions/P1_SUBMIT_71779.txt)。

当前MMSI27B片0=`71064_0`、1=`71095_1`、2=`71114_2`运行，10:16分别保存31/100、37/100、25/100，近期日志持续更新且未见新错误；片3=`71779_3`、片4–7=`71510_[4–7%1]`排队，同组跨数组上限仍为5。72B=`70202_0`继续等3×H20，原TP2+独占SVC1和显式.93不变。账户3/5运行、6×A100、10/10提交槽（含无关55278）；8–9待空槽。正确P1仍2/4组全量完成，无新增完成片。8192/no-thinking/BF16/65536及官方图片顺序/SVC设置不变，不重配环境或新增smoke。排队和长回复使全量完成时间仍无法可靠估计。

## 历史进展：2026-09-14 20:10 正确P1第二组全量完成，MMSI27B三片运行

MindCube/9B末片9=`71002_9`为51/95（9:57:58），片10=`71041_10`为42/100（8:20:57）；11片严格合并通过，**453/1050（43.14%）**。among234/600、around122/250、rotation97/200，无重复遗漏、0skip，COMPLETE哈希与固定run-group一致。44道r10保存答案、1006道r11答案逐题注明，不混入1024预算。原始解析器未改；26次最终答案解析失败及868次评分解析异常事件原样保留，不因解析错误删题或重跑挑选答案。[全量核验与全部日志索引](./records/P1_MINDCUBE9B_FULL_20260914.json)。末两片195/195原始QA、结果/COMPLETE字节一致、gzip完整性及无凭据检查通过，服务器原件保留。正确P1已完成2/4组。

MMSI/27B片0=`71064_0`、1=`71095_1`、2=`71114_2`实际运行（每片2×A10080），19:48分别已保存7/100、3/100、3/100（片0含2道导入答案），日志显示SVC生成及模型回复。片3=`71151_3`仍排队。利用释放的2个提交槽，确认71345_[4–5]尚未启动后hold并仅取消PENDING，以`71510_[4–7%1]`替换；旧4–5排队年龄重计，所有旧记录/检查点保留，无健康任务被重启。该组跨数组上限1+1+1+1+1=5、无完成依赖，片8–9待空槽。[实际提交输出](./submissions/P1_SUBMIT_71510.txt)。

MMSI/72B=`70202_0`继续等3×H20，保持TP2+独占SVC1及原sbatch显式GPU_MEMORY_UTILIZATION=0.93。账户当前3/5运行、6×A100、10/10提交槽（含不属于本任务的55278）；19:43兼容A100节点11/12/15及H20节点16均8/8GPU已分配。提交槽满不等于运行额度满。8192/no-thinking/BF16/65536、官方图片顺序、论文对齐SVC多图适配参数不变，未重配环境或添加import/version/smoke。MMSI早期每片样本太少且生成有长回复，队列等待未知，目前无可靠全P1完成时间；继续集中推进MMSI27B，有空槽补齐8–9。

## 历史进展：2026-09-14 14:55 MindCube/9B第9片完成，补交MMSI片5

MindCube/9B片8=`71002_8`完成，耗时7:02:57。冻结验证器确认95题完整覆盖、44正确/51错误、无重复遗漏、0skip，COMPLETE及结果SHA一致、run-group及8192/no-thinking/BF16/65536/SVC设置与既有片相同。该片95题均由r11新生成，无导入答案。累计0–8共9/11片，部分 **360/855（42.11%）**，非全量成绩。[核验及完整原始日志](./records/P1_9B_SHARD8_20260914.json)已归档：95/95原始QA、结果/COMPLETE逐字节、gzip完整性、无凭据检查通过。服务器源文件均保留。

只剩9=`71002_9`、10=`71041_10`运行，14:42已保存72/95、44/100。释放槽用于MMSI/27B片5：精确确认旧片4=`71187_4`仍PENDING/零运行时间后，先hold并仅取消PENDING，再把片4–5合并提交`71345`（4-5%1）。其他片0–3各%1，全部活动数组合计5，无完成依赖、无重复提交；此限流并非要求固定前一片完成才可提交。旧片4的排队年龄重新计算，原提交记录和检查点不删。实际提交见[记录](./submissions/P1_SUBMIT_71345.txt)。72B70202_0保持3H20/TP2+独占SVC1/.93，健康推理均未重启。

2运行/4×A100、10/10提交槽。节点11因其他作业71040的batch退出失败被管理员置为MIXED+DRAIN，即使余2卡也不能接新任务；节点12余2卡但PLANNED/待调度任务显示Priority，节点15和H20均满。不修改管理员节点状态或绕过调度，不重配环境/重复import/version/smoke。继续集中完成9B并推进MMSI，排队仍使全量ETA不可靠。

## 历史进展：2026-09-14 10:55 MindCube/9B第8片完成，末三片运行

MindCube/9B片7=`71002_7`完成，耗时8:40:23。冻结验证器确认95题完整覆盖、30正确/65错误、无重复遗漏、0skip；COMPLETE与结果SHA一致，与既有片run-group相同。8192/no-thinking/BF16/65536及SVC设置不变。95题均由r11新生成，无导入答案。累计完成0–7共8/11片，部分 **316/760（41.58%）**，不是全量成绩。[核验及完整日志索引](./records/P1_9B_SHARD7_20260914.json)。完整日志已通过95/95原始QA覆盖、结果/COMPLETE逐字节检查、gzip完整性和无凭据扫描，服务器原件全部保留。

最后8/9=`71002_8/9`、10=`71041_10`均运行。片10复用现有节点本地环境，确认SVC生成、评分请求HTTP200和vLLM约87 token/s，不把启动状态冒称已完成首题。跨数组上限4+1=5，实际仅剩3片。已用释放槽补交MMSI/27B片4=`71187_4`，与71064_0、71095_1、71114_2、71151_3合计最大5并发。[实际提交输出](./submissions/P1_SUBMIT_71187.txt)。下一次扩片必须将该组所有活动数组合计上限维持≤5，不能再简单增加第6个%1。72B70202_0保持3H20/TP2+独占SVC1/显式.93。

当前3运行/6×A100、10/10提交槽，兼容A100与H20均已分配，其余继续排队。未重启健康任务、未重配环境或重复import/version/smoke。继续集中完成9B并推进MMSI；排队时间未知，暂无可靠全P1完成时间。

## 历史进展：2026-09-14 10:00 MindCube/9B第7片完成，补交MMSI片3

MindCube/9B片6=`71002_6`完成，耗时8:03:29。冻结验证器确认95题完整覆盖、37正确/58错误、无重复遗漏、0skip；COMPLETE与结果SHA一致，与既有片run-group完全一致。8192/no-thinking/BF16/65536及SVC设置不变。该片95题均由r11新生成，没有导入答案。累计完成0–6共7/11片，部分 **286/665（43.01%）**，不是全量成绩。[核验及完整日志索引](./records/P1_9B_SHARD6_20260914.json)。日志包已通过95/95原始QA覆盖、结果/COMPLETE逐字节检查、gzip完整性和无凭据扫描，服务器源文件全部保留。

当前同组7/8/9运行、10排队；71002上限4加71041上限1合计5（实际仅剩4片）。释放的提交槽已补交MMSI/27B片3=`71151_3`，与71064_0、71095_1、71114_2合计最多4并发。[实际提交输出](./submissions/P1_SUBMIT_71151.txt)。提交前排除完成、运行和排队的重复片，沿用r11 Run ID/源码与配置。72B70202_0保持TP2+独占SVC1共3H20及显式.93不变。

当前3运行/6×A100、10/10提交槽；兼容A100和H20均已分配，其余任务继续等待资源或Priority。未重启健康作业、未重配环境或重复import/version/smoke。继续集中完成9B，有空槽推进MMSI4–9；排队时间未知，暂无可靠全量ETA。

## 历史进展：2026-09-14 07:10 MindCube/9B第6片完成，补交MMSI片2

MindCube/9B片5=`71002_5`完成，耗时6:05:57。冻结验证器确认95题完整覆盖、34正确/61错误、无重复遗漏、0skip；COMPLETE与结果SHA一致，与前片run-group完全一致。8192/no-thinking/BF16/65536及SVC设置不变。保留r10导入17题和r11新增78题的来源、原始检查点与完整日志。累计完成0–5共6/11片，部分 **249/570（43.68%）**，不是全量成绩。[核验及日志索引](./records/P1_9B_SHARD5_20260914.json)。完整日志包已通过95/95原始QA覆盖、结果/COMPLETE/检查点逐字节检查、gzip完整性和无凭据扫描，服务器源文件全部保留。

同组片8已接上运行，当前6/7/8运行、9/10排队；跨数组上限71002%4+71041%1=5保持。释放的提交槽已补交MMSI/27B片2=`71114_2`，与71064_0、71095_1合计最多3并发。[实际提交输出](./submissions/P1_SUBMIT_71114.txt)。提交前已排除完成、运行和排队的重复片，沿用r11 Run ID/源码与配置。72B70202_0保持TP2+独占SVC1共3H20及显式.93不变。

当前3运行/6×A100、10/10提交槽；兼容节点仅余1张A100，其余GPU已占用。未重启健康作业、未重配环境或重复import/version/smoke。优先集中完成9B，后续空槽继续推进MMSI3–9；仍无法可靠预测排队时间。

## 历史进展：2026-09-14 04:38 MindCube/9B第5片完成，补交MMSI片1

MindCube/9B片4=`70926_4`完成，耗时8:51:56。冻结验证器确认95题完整覆盖、47正确/48错误、无重复遗漏、0skip、COMPLETE与结果SHA一致；与前四片run-group完全一致，8192/no-thinking/BF16/65536和SVC设置不变。保留r10导入1题及r11新增94题的原始来源。累计完成0–4共5/11片，部分 **215/475（45.26%）**，不代表全量成绩。[核验及完整日志索引](./records/P1_9B_SHARD4_20260914.json)。日志包已核对95/95原始QA、结果/COMPLETE/原始检查点、gzip完整性及无凭据，服务器所有源文件保留。

将71002数组上限从3调至4，与71041上限1合计5；已完成70926不计入活动并发。5–7运行、8–10排队，每片2×A10080。空出的提交槽用于MMSI/27B片1=`71095_1`，片0仍为71064_0；该组合已有相同设置的r10成功答案，按用户授权的非致命告警版本r11并行续跑，不新增smoke。[实际提交记录](./submissions/P1_SUBMIT_71095.txt)。两个数组合计上限2，账户global仍最多5运行/10提交。72B70202_0保持3×H20/TP2+独占SVC1/.93排队。

当前3运行/6×A100、10/10提交槽。节点11虽有空闲GPU/8CPU但处于Slurm计划分配状态，待运行任务仍受Priority限制；节点15仅余1GPU，其余兼容A100和H20已满。不修改健康任务或实验参数、不重配环境。继续集中完成9B并用后续空槽推进MMSI；排队未知，暂无可靠全量ETA。

## 历史进展：2026-09-14 02:23 正确P1首组全量完成，补交MMSI27B

MindCube/Qwen3.8-27B最后片10=`70775_10`完成，48/100，耗时7:40:11。冻结验证器严格合并通过：11片、1050题完整覆盖、无重复遗漏、0 skip；全量 **578/1050（55.05%）**，among=257/600、around=150/250、rotation=171/200。逐题来源明确保留r10的358题及r11的692题，不混入1024结果。[全量核验及日志索引](./records/P1_MINDCUBE38_FULL_20260914.json)；[分类成绩](./RESULTS.md#qwen38-27bp1)。末片完整日志包964902字节/518文件，哈希、gzip完整性、无凭据扫描、100/100原始QA以及结果/COMPLETE逐字节检查通过；先前各片日志保留。

已实际补交MMSI/Qwen3.5-27B首片=`71064_0`，2×A10080，沿用r11 Run ID、8192/no-thinking/BF16/65536以及固定SVC配置；[实际sbatch提交输出](./submissions/P1_SUBMIT_71064.txt)。不重配环境、不增加import/version/smoke，不重启健康作业。MMSI/72B=`70202_0`保持TP2+独占SVC共3×H20、GPU memory utilization=0.93排队。

MindCube/9B：0–3已完成（168/380，仅部分成绩），4=`70926_4`、5–7=`71002_5/6/7`运行，8–9=`71002_8/9`、10=`71041_10`排队，每片2×A100。该组跨数组并发上限1+3+1=5。账户实际4/5运行、8张A100、10/10提交槽（包含不属于本任务的55278，不修改）。H20及兼容A100资源仍受占用，不能把提交槽已满说成GPU运行额度已满。优先完成9B，释放提交槽后继续推进MMSI。排队时间未知，暂不给出无依据的全P1完成时间。

## 历史进展：2026-09-14 01:10 9B第4片完成，最后待提交片已排71041

MindCube/9B片3=`70825_3`完成（6:21:35），严格验证35/95、无重复遗漏、0skip，COMPLETE和结果SHA匹配；源码/run-group与既有同组一致，8192/BF16/65536/thinking=false和SVC设置不变。保留r10导入3题和r11新生成92题的明确来源、导入前原始检查点。累计完成0–3共4/11片，部分成绩**168/380（44.21%）**，不是全量成绩。[核验及来源记录](./records/P1_9B_SHARD3_20260914.json)；[完整原始日志包](./raw_logs/mindcube-qwen35-9b-shard3-raw-20260914.tar.gz)。

已补交最后未提交的9B片10=`71041_10`，每片2×A10080；[实际提交输出](./submissions/P1_SUBMIT_71041.txt)。当前9B片4/5运行、6–9排71002、10排71041，活动数组上限1+3+1=5。全部MindCube分片均已完成或提交；下一空余提交槽推进MMSI27B片0。MindCube/3.8最后片10仍运行84/100，9B片4为57/95，片5确认实际推理约87.5 token/s。3运行/6×A100，10/10提交槽满；CPU/GPU资源组合仍不足，其余排队。健康作业不重启，环境和实验设置不变。

## 历史进展：2026-09-14 00:10 五片完整日志补归档完成

之前共享目录读取等待的`70825_3.1`归档步骤已正常完成（23分04秒），没有重启推理。已取回[完整日志包](./raw_logs/mindcube-completed-batch2345-20260913.tar.gz)：5,819,279字节、2,350文件，SHA256为`a3e9699aceb6429c8777fac31fc5ac750916c7830624f2886f96465207ef0fed`。gzip完整性、凭据扫描通过；五片475/475道题原始QA回答全部覆盖，results/COMPLETE及9B导入前检查点与已归档文件逐字节一致，完整pipeline/vLLM日志齐全。原始服务端文件全部保留。[更新后的归档验证记录](./records/P1_COMPLETED_BATCH_20260913_2345.json)。

无新增实验完成或失败：MindCube/3.8片10为72/100，9B片3/4为83/95、53/95；3运行/6×A100、10/10提交槽。71002片5–9仍等Priority，72B片0等Resources；节点11剩余CPU不足以满足当前任务8CPU要求，其余兼容GPU已分配，不更改健康作业或实验设置。下一空余提交槽仍优先9B片10，再推进MMSI27B。

## 历史进展：2026-09-13 23:52 新增5片完成，补交71002

新增严格验证完成：MindCube/3.8片7=46/95、片8=53/95；MindCube/9B片0=44/95、片1=45/95、片2=44/95。逐片通过冻结r11验证器的完整覆盖/无重复遗漏/0skip检查，结果与COMPLETE哈希及8192/BF16/65536/thinking=false等参数核对通过。9B三片分别保留r10导入4/2/17题的原始检查点和逐题来源，不冒称全部r11新生成。[五片验证与原始结果链接](./records/P1_COMPLETED_BATCH_20260913_2345.json)。

部分成绩：MindCube/3.8完成0–9共10/11片，**530/950（55.79%）**，仅余片10=`70775_10`运行；MindCube/9B完成3/11片，**133/285（46.67%）**，片3/4继续运行。小型results/COMPLETE及原始检查点已取回保存；完整原始响应与日志包仍在计算节点低CPU归档，读取共享子目录时出现D状态等待，尚不能宣称日志包已完整归档或校验。所有原件保留，归档步骤`70825_3.1`及恢复信息记入清单；后续完成后补传并核验，不重复创建同名归档。

已实际补交9B片5–9为`71002`（%3），与70825/70926各%1合计最大5并发。[实际提交输出](./submissions/P1_SUBMIT_71002.txt)。当前3运行/6×A100、10/10提交；新片等待Priority，72B70202_0等待Resources。节点11虽余3张A100，但仅余6CPU、任务需8CPU；其余兼容A100和H20均满，不将空闲GPU误报为可立即启动资源。下一提交槽优先9B片10，然后MMSI27B；不修改健康任务、环境或实验设置。

## 历史进展：2026-09-13 18:07 第8片已核验完成，补交70926

MindCube/3.8片9=`70765_9`完成，49/95，运行5:53:38。严格单片覆盖/无重复遗漏/0skip验证通过，COMPLETE与results哈希匹配，run-group、源码及8192/BF16/65536/thinking=false等参数与既有r11一致。95题均为r11新生成，无导入答案。片0–6及9共8/11片，部分成绩**431/760（56.71%）**，不是全量成绩。[本片核验](./records/P1_COMPLETED_SHARD9_20260913.json)及[完整原始日志](./raw_logs/mindcube-qwen38-shard9-raw-20260913.tar.gz)已保存，510文件/860171字节，所有服务器原件保留。

当前3.8片7/8运行（80/95、91/95），10排队70775；9B片0/1/2运行（0/1已90/95、58/95），片3排队70825，新增片4=`70926_4`。9B跨数组合计3+1+1=5，账户全局仍最多5运行/10提交；补交后额度满，使用10×A10080。72B70202_0继续排队，MMSI27B待提交槽。没有新失败，没有重启健康作业，没有改设置或重复环境检查。

## 历史进展：2026-09-13 14:53 第7片已核验完成，补交70825

MindCube/3.8片6=`70438_6`完成，严格验证95题、55正确/40错误、无遗漏重复、0skip；结果与COMPLETE SHA匹配，同组指纹与既有r11一致，8192/BF16/65536/thinking=false及原SVC参数不变。该片95题均由r11新生成，没有导入答案。片0–6累计**382/665（57.44%）**，完成7/11片，仅为部分成绩。[本片核验/归档记录](./records/P1_COMPLETED_SHARD6_20260913.json)；[完整原始日志包](./raw_logs/mindcube-qwen38-shard6-raw-20260913.tar.gz)包含520文件、873533字节，服务器原件全部保留。

当前MindCube/3.8片7/8/9运行、10=`70775_10`排队；MindCube/9B片0/1运行、2排队70766，利用释放的提交槽补交片3=`70825_3`。9B跨数组上限3+1=4，主组名义上限2+2+1=5且只剩4片，账户仍限制5运行/10提交。MMSI/72B片0继续排队，MMSI/27B待后续槽位。五个作业使用10×A10080，兼容A100/H20当前均已分配。未重启健康作业、未改设置、未重配环境或新增smoke检查。

## 历史进展：2026-09-13 12:05 已占满5个运行名额，重排端口冲突片

MindCube/3.8片6=`70438_6`78/95、7=`70438_7`9/95、8=`70765_8`7/95、9=`70765_9`2/95，均已有实际推理产物。MindCube/9B片0=`70766_0`刚启动，日志确认复用已有节点本地环境，尚未确认新答案。共5/5运行、10×A10080；9B片1/2和MMSI/72B片0继续排队，MMSI/27B待后续提交槽位。完成片仍为MindCube/3.8的0–5，327/570仅代表完成部分。

`70765_10`启动2秒后因`TCP port 30775 is already in use`退出，发生在模型加载前，不是共享存储故障。本次仅将该失败片重排为`70775_10`，沿用原Run ID、r11固定源码及参数，旧日志保留。原70765并发上限降至2，加70438上限2和70775上限1，主组跨数组合计仍为5；没有取消或重启健康作业。重提后共10/10提交任务（含无关55278）。8192、BF16、65536、no-thinking、图序及SVC设置未改；未重配环境或执行额外import/smoke检查。

## 历史进展：2026-09-13 11:05 六片已核验完成，补交70765/70766

重新连通DAAI后确认MindCube/3.8片0–5全部完成且通过严格单片验证：覆盖各95题、无重复遗漏、0skip、8192/BF16/65536/thinking=false、源指纹及COMPLETE SHA一致。旧r10片1/3/4先保留原始结果/COMPLETE，再按明确逐题来源导入r11并重新验证，生成新COMPLETE，不占GPU重跑。r11片0/2/5实际新增56/61/95题，共212题；各抽一题原图→staging→QA顺序一致。[完整核验及来源记录](./records/P1_AUDITFIX_COMPLETED_20260913_1055.json)。

| 分片 | 正确 / 总数 | 产物来源 |
|---|---:|---|
| 0 | 53 / 95 | r10导入39题，r11新增56题 |
| 1 | 48 / 95 | r10完整95题，带来源导入r11 |
| 2 | 63 / 95 | r10导入34题，r11新增61题 |
| 3 | 52 / 95 | r10完整95题，带来源导入r11 |
| 4 | 53 / 95 | r10完整95题，带来源导入r11 |
| 5 | 58 / 95 | r11完整95题 |
| 已完成部分 | 327 / 570（57.37%） | 6/11片，非全量成绩 |

实际补交MindCube/3.8末三片8–10为`70765`（%3），与70438剩余6/7（%2）合计上限5，该组全部11片现已完成或提交。再以MindCube/9B片0–2=`70766`（%3）填候补槽，账户仍最多5运行/10提交。最新运行6=`70438_6`68/95、7=`70438_7`3/95、8=`70765_8`1/95，共6×A100；9/10、9B0/1/2及72B0排队，含无关55278共10/10提交。MMSI/27B续接仍待后续槽位；72B仍等3×H20。主组上限跨全部活动数组计算，不对已完成片重复提交。

原始results、COMPLETE、导入前检查点及验证记录已逐片归档。[完整日志压缩包](./raw_logs/mindcube-qwen38-shards0-5-raw-20260913.tar.gz)含4,981个文件、7,530,345字节，包括完整attempt日志、原始QA/评分JSON、图序记录及来源；[归档哈希清单](./records/P1_AUDITFIX_RAW_ARCHIVE_20260913.json)已核对。所有源文件保留，未复制权重/视频或重配环境。单CPU归档复用既有计算节点，不新占GPU。没有更改推理设置或重启健康作业。后续优先完成MindCube/3.8，再利用释放资源推进其他正确P1；排队变化仍使全量ETA不可靠。

## 历史进展：2026-09-13 00:26 修正版首片已开始实际推理

`70438_0`已在hkbugpusrv11获得2×A100，实际attempt为70518。日志确认直接复用`/dev/shm/mj-p1-1194/env-4c3ccc5763b452719ee9564c6afbff63c9e13bfc1f0b6513ddfbfa0b4e5dec50`，没有重新分发环境。SVC生成及两次评分请求已完成，当前进入下一题最终QA，vLLM约28.6 token/s。仍保存39/95，全部为导入旧答案，不能把这个数当成修正版新答题数；还没有新完整题可验证。启动源码指纹为r11，参数记录8192/BF16/65536/thinking=false，原检查点成功接续。[真实启动/生成日志与参数快照](./records/P1_AUDITFIX_PROGRESS_20260913_0026.json)。

旧健康片1/3/4继续，分别74/95、68/95、68/95。当前同组4运行、8×A100；r11片2/5/6/7和MMSI/72B片0仍排队，含无关55278共10/10提交。兼容节点11/12/15及H20节点16 GPU均已分配，没有足够空闲GPU立即补第5位；数组%2加旧3片仍总上限5，不调整节流或重复提交。无新增失败、无完整分片、无新全量成绩；健康作业与旧产物不动，自动推进继续。

## 历史进展：2026-09-12 23:01 用户授权告警修复，已重提70438

用户明确同意：额外thinking输出检查不应中断整片，改为告警及完整响应日志，继续按原评分协议处理不删改的回答。该检查是适配时额外添加，不是原仓库要求。此次仅`utils/api.py`在固定源码指纹文件集中改变，客户端/服务端关闭thinking、8192、BF16、65536、采样/提示词/评分、官方图片顺序及SVC设置不变；API/网络等真实错误仍正常报告。之前的“等待诊断确认/暂停扩片”限制已由此授权取代。

独立冻结r11：commit `dac49f6db2f0873134b6905ac2244e4284c09948`，source SHA256 `72626718674f0cdc326e3e80a2e8c97b79b88aa2fad212e9b37cd2a61d6873fa`，目录`/home/comp/24482277/shiyang/MindJourney-p1-output8192-r11-20260912`。不修改r10或重启健康作业。14项相关本地测试通过，没有重装环境/import/version/smoke或扫描模型权重。[新执行清单、实际sbatch与续接来源](./records/P1_AUDITFIX_EXECUTION_20260912.json)。

已实际提交数组`70438`：MindCube/3.8片0、2、5、6、7，%2、每片2×A10080。原片5=`70281_5`仅在PENDING状态下取消并替换，日志保留；片0/2续接39/34题。3个健康r10片1/3/4与r11数组合计最大并发5。提交前检查无重复，当前10/10提交任务（含无关55278），兼容A100与H20均已分配，因此新数组尚在等待GPU，不冒称已开始推理。

| 正确P1实验 | 23:01实际运行 | 已排队 | 每片GPU |
|---|---|---|---|
| MindCube / Qwen3.8-27B | 1=`70263_1` 63/95；3=`70271_3` 52/95；4=`70281_4` 54/95 | r11 0/2/5/6/7=`70438` | 2×A10080 |
| MMSI / Qwen2.5-VL-72B | 无 | 0=`70202_0`（保留r10，无此guard） | 3×H20，TP2+SVC1，0.93 |
| MindCube / Qwen3.5-9B | 无 | r11已准备，待槽位 | 2×A10080 |
| MMSI / Qwen3.5-27B | 无 | r11已准备，待槽位 | 2×A10080 |

MindCube/9B旧0–5片已保存44题、MMSI/27B已保存7题，均已续接准备，不重复生成已保存答案；无原检查点的片从头正常运行。原始results及原始实验fingerprint保存在`resume_original_results.json`与`resume_lineage`，完成题原始文件只读链接、得分及随机状态原样保留。r11元数据明确区分“旧源码生成的导入题”与“修正版续生成题”，不得只报r11而隐去来源。旧r9/1024不混入。

自动监控已更新：优先保持同组总并发5，健康旧片结束后适当提高70438数组上限；提交槽释放先补本组8–10，再以其他正确P1补位。旧健康片完成先严格验证归档，再带来源导入r11；若失败则续接剩余题，不再用旧致命guard重跑。每片完成立即保存、推送，全部四组完成前不停止。[最新原始运行快照](./records/P1_AUDITFIX_PROGRESS_20260912_2301.json)。

当前健康片平均每题约4–5分钟，仅粗估它们还需约2.5–3小时；新分片等待资源且修正版尚无实际首题，无法可靠承诺四项P1全量完成时间。

## 历史进展：2026-09-12 21:25分片2失败；21:46轮次连接受阻

21:25核验：MindCube/3.8片2=`70271_2`在3:02:28后FAILED 1:0，保存34/95（答案解析错误2、0skip）。失败栈为最终`answer_scaling`响应触发thinking标签检查，与片0同类，不是本次新增I/O/OOM证据。参数记录仍8192、thinking=false及固定r10源码指纹；错误文案不证明开关未传。见[原始日志尾部与结果指纹](./records/P1_OUTPUT8192_PROGRESS_20260912_2125.json)。

该时刻片1/3/4（70263_1、70271_3、70281_4）运行，分别42/95、28/95、29/95，每片2×A100，共3作业/6卡；片5=`70281_5`与72B片0=`70202_0`排队，含无关55278共6个提交任务。兼容GPU已分配，无可立即利用的两卡/三卡资源。MC3.8剩余数组有效合计最大并发3，不新增、不原样重试失败片，健康作业及已有队列保留。三个Qwen3组合的扩片均暂停；独立原始响应日志诊断仍待确认。没有完整8192分片，不报告新全量成绩或可靠ETA。

21:46触发的监控轮次中，原SSH会话失效；重连经历跳板认证失败、banner超时及连接关闭，未取得新的实时队列。因此以上题数/资源状态仅代表21:25，不能视为本轮实时数据。没有因连接问题修改、取消或重启作业，也未重配环境/扫描权重。此次补归档上一轮已取得的失败证据，监控继续。

## 历史进展：2026-09-12 20:05 MindCube/3.8也触发标签检查

MindCube/3.8首片=`70207_0`在3:39:53后FAILED 1:0，保存39/95（答案解析错误3）。这次发生在最终`answer_scaling`请求而非评分请求，guard检测到thinking标签后退出。关闭服务的SIGTERM是失败后的清理，不作为外部取消或OOM证据。至此三个Qwen3组合均出现标签检查失败。保存产物参数仍8192、thinking=false和固定r10指纹，不能仅凭异常文案断言开关没传。详见[失败栈、参数与进度快照](./records/P1_OUTPUT8192_PROGRESS_20260912_2005.json)。
...
MMSI/27B片4=`70219_4`在1:36:28后因评分响应thinking标签检查FAILED 1:0，已保存3/100。至此MMSI/27B和MindCube/9B两组各自已提交0–5六片均失败，没有任何完整分片。失败现场保留，不重试、不放宽检查、不修改参数；仅增原始响应日志的独立诊断版本仍等待用户确认。见[最新失败与运行证据](./records/P1_OUTPUT8192_PROGRESS_20260912_1919.json)。
...
MMSI/27B片5=`70250_5`在1:18:20后因评分响应thinking标签检查FAILED 1:0，已保存1/100；该组累计0/1/2/3/5失败，只有片4=`70219_4`仍运行（2/100）。保留全部现场、不重试、不放宽检查，独立诊断仍等待用户确认。没有新增I/O/OOM证据。见[失败日志与进度快照](./records/P1_OUTPUT8192_PROGRESS_20260912_1905.json)。
...
MMSI/27B片1=`70214_1`（1:02:27）和片2=`70214_2`（5:38）在评分阶段因thinking标签检查FAILED 1:0；该组累计0–3四片失败，4/5仍运行。两次失败日志与部分结果快照见[18:16记录](./records/P1_OUTPUT8192_PROGRESS_20260912_1816.json)。未把错误文案当作开关未传的证明，未重试或修改冻结r10；独立原始响应日志诊断仍等待用户确认。
...
新增失败：MindCube/9B片5=`70200_5`（1:29:26，17/95）和MMSI/27B片3=`70219_3`（10:56，1/100），均在评分阶段被thinking标签检查拦截，FAILED 1:0；不是新I/O/OOM证据。9B已提交0–5六片现全部失败，MMSI/27B累计0/3两片失败，均保留现场、暂停扩片且不原样重试。错误文案不证明enable_thinking开关未传；上一轮重建请求没有复现标签错误，不能据此放宽检查。详见[进度及失败日志](./records/P1_OUTPUT8192_PROGRESS_20260912_1747.json)。
...
MindCube/9B新增片0、4在评分请求后触发同一thinking标签检查；累计0/1/3/4四片FAILED 1:0，均不原样重试。片2、5仍运行，不取消健康作业。16:28快照片0/1/2/3/4/5分别4/2/15/3/1/3题（每片95题），无完整片。MMSI/27B `70201_0`、MindCube/3.8 `70207_0`已经获得GPU接替启动；MindCube/3.8已保存3题且答案解析错误为0。新进度见[快照](./records/P1_OUTPUT8192_PROGRESS_20260912_1630.json)，仅为部分题数，不报告全量成绩。
...
MindCube/9B实际结果记录8192、thinking=false、BF16、65536及固定r10源指纹；五个已有产物分片各抽查一题，原图→staging→最终QA图片顺序一致，已保存的抽查回答无thinking标签。当前快照：片0=3/95、片1=2/95、片2=6/95、片3=3/95、片4=1/95；片5刚启动未保存首题。0skip，尚无完整分片。详见[实时核验和失败说明](./records/P1_OUTPUT8192_LIVE_AUDIT_20260912_1615.json)。
...
当前4片获得8张A100、10/10提交槽位（含无关55278）。策略仍优先用满账户额度，再集中同组；其他已排队组合可填第5运行位。新作业组合/数据/源指纹检查通过；此时尚未核验8192正式首题，不把Slurm RUNNING冒称为推理成功。完整参数、提交stdout、launch_manifest见[8192执行清单](./records/P1_OUTPUT8192_EXECUTION_20260912.json)。

本次参数切换已停止旧数组70165/70166/70167/70168/70179；Slurm子步骤FAILED 15:0来自主动终止，不是新I/O故障。旧结果全部保留原路径：MindCube/9B三片52/95、45/95、54/95；MindCube/3.8为32/95；MMSI/27B为16/100。旧产物路径及SHA见[迁移保留记录](./records/P1_OUTPUT8192_MIGRATION_20260912.json)。历史完整6组结果不删除、不修改得分。旧任务的ETA失效，等待8192题速后重新估计。
...
五片结果及日志均近期更新，0 skip，Slurm 无新增失败，日志末尾无 I/O/OOM 错误。尚无完整分片，不提前生成 COMPLETE 或全量结果。固定源指纹、BF16/no-thinking/65536 保持一致。[本轮监控快照](./records/P1_PROGRESS_20260912_1546.json)。
...
三组已经实际推理，首片快照：MindCube/9B 5/95、MindCube/3.8-27B 5/95、MMSI/27B 2/100；72B 仍等待 H20。已核对这 12 道完成题目的原始题目、图片 staging 顺序与实际 QA prompt 图片顺序，均一致；运行记录的固定源码、BF16、no-thinking、65536 未变化。详见[实时产物核验摘要](./records/P1_LIVE_AUDIT_20260912_1355.json)。分片未完成，以上不作为全量成绩。
...
原执行 JSON 的 submit_args 漏记初始提交继承的缓存环境变量，首次补提在 sbatch 前因默认缓存缺失被拒绝，未生成作业、未改变运行中任务。补齐原始 `P1_CACHE_ROOT` 和 `P1_MODEL_VALIDATION_ROOT` 后成功复用既有缓存，未重新配环境。已将这两个必需变量补入[执行清单](./records/P1_CORRECTED_EXECUTION_20260912.json)，后续续提必须一并传入。
...
以上已由 Slurm 接收，但尚未开始推理，首题产物核验尚未发生；排队不等于成功运行。72B 实际提交 export 明确为 `P1_GPU_MEMORY_UTILIZATION=0.93`。完整 Run ID、固定数据路径、提交参数与归档目录见[执行清单](./records/P1_CORRECTED_EXECUTION_20260912.json)及各新目录的 `submission_initial.txt`。账户目前 5 个排队任务（含无关55278），没有取消无关作业。
...
目录整理（2026-09-14）：顶层仅保留 Markdown 和 SHA 清单；执行/核验/进度 JSON 已移至 `records/`，提交输出已移至 `submissions/`。当前执行清单为 [records/P1_AUDITFIX_EXECUTION_20260912.json](./records/P1_AUDITFIX_EXECUTION_20260912.json)。后续归档请沿用此目录结构；数据集产物、Run ID 和服务器路径不变。
