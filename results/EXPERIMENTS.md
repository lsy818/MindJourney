...
纠错：用户没有变更清单，是我将 MindCube/MMSI 的执行组合配反。下文历史“四项 P1 全部完成”仅表示错误组合完成，该结论对于用户要求的 P1 撤回。旧结果全部保留；正确清单见[全部实验结果汇总](./RESULTS.md)，重新启动前的检查见[重启核对记录](./P1_RESTART_AUDIT_20260912.md)。
...
## 最新进展：2026-09-12 20:05 MindCube/3.8也触发标签检查

MindCube/3.8首片=`70207_0`在3:39:53后FAILED 1:0，保存39/95（答案解析错误3）。这次发生在最终`answer_scaling`请求而非评分请求，guard检测到thinking标签后退出。关闭服务的SIGTERM是失败后的清理，不作为外部取消或OOM证据。至此三个Qwen3组合均出现标签检查失败。保存产物参数仍8192、thinking=false和固定r10指纹，不能仅凭异常文案断言开关没传。详见[失败栈、参数与进度快照](./P1_OUTPUT8192_PROGRESS_20260912_2005.json)。

停止新增MindCube/3.8分片，不原样重试，不修改冻结r10、不剥离标签或放宽guard。健康片1/2/3/4继续（70263_1、70271_2、70271_3、70281_4），分别28/95、21/95、8/95、13/95；保留已排片5=`70281_5`和72B片0=`70202_0`。当前4运行、8×A100；剩余MindCube数组合计上限4，片5受其数组%1限制。兼容GPU及H20均已分配，不能立即补第5作业。含无关55278提交7/10，无重复提交、无环境/权重重检。全部结果日志保留，未有完整分片。

此前仅增加原始失败响应日志的独立诊断版本尚未获用户确认。本轮不把监控授权当作允许改推理语义；先保留健康作业和已排72B，诊断/修复需要的独立版本继续等待确认。全量ETA不可靠。

## 历史进展：2026-09-12 19:19 MMSI已提交六片全部失败，五片集中跑MindCube/3.8

MMSI/27B片4=`70219_4`在1:36:28后因评分响应thinking标签检查FAILED 1:0，已保存3/100。至此MMSI/27B和MindCube/9B两组各自已提交0–5六片均失败，没有任何完整分片。失败现场保留，不重试、不放宽检查、不修改参数；仅增原始响应日志的独立诊断版本仍等待用户确认。见[最新失败与运行证据](./P1_OUTPUT8192_PROGRESS_20260912_1919.json)。

MindCube/3.8片4=`70281_4`已自动获得GPU，现片0/1/2/3/4共5片并行（70207_0、70263_1、70271_2、70271_3、70281_4），5/5运行、10×A100集中同组。片0/1/2分别34/95、20/95、10/95；片3处于实际生成中但尚未保存首题，片4刚启动。排队片5=`70281_5`及MMSI/72B片0=`70202_0`，含无关55278共8/10提交。当前已满运行额度且MindCube跨数组上限5，不追加造成上限超过5的新数组。仍保持BF16、8192、no-thinking、65536、固定r10和官方图序，0skip；没有完整分片可归档为COMPLETE。

## 历史进展：2026-09-12 19:05 MMSI片5失败，MindCube/3.8四片运行

MMSI/27B片5=`70250_5`在1:18:20后因评分响应thinking标签检查FAILED 1:0，已保存1/100；该组累计0/1/2/3/5失败，只有片4=`70219_4`仍运行（2/100）。保留全部现场、不重试、不放宽检查，独立诊断仍等待用户确认。没有新增I/O/OOM证据。见[失败日志与进度快照](./P1_OUTPUT8192_PROGRESS_20260912_1905.json)。

MindCube/3.8片3=`70271_3`已自动接替获GPU，与片0/1/2一起运行；片0/1/2分别30/95、18/95、10/95，片3尚无已保存答案。保存产物8192/thinking=false/固定r10源指纹，0skip；片0答案解析错误累计2，片1/2为0。无完整分片，不产生新全量成绩。

当前仍5/5运行、10×A100；排队MindCube/3.8片4/5=`70281_4`/`70281_5`和MMSI/72B片0=`70202_0`，含无关55278共9/10提交。MindCube/3.8已有跨数组最大并发5、账户运行额度已满，本轮不再新建数组突破该组上限；现有候补足够接替，待数组完成释放并发额度后补后续片。H20全满，A10011/15各单卡空余不足一片。健康作业不取消、不重启、不重复环境或资源校验。全量ETA仍取决于失败诊断，不能可靠承诺。

## 历史进展：2026-09-12 18:17 MMSI新增两片失败，继续满额推进MindCube/3.8

MMSI/27B片1=`70214_1`（1:02:27）和片2=`70214_2`（5:38）在评分阶段因thinking标签检查FAILED 1:0；该组累计0–3四片失败，4/5仍运行。两次失败日志与部分结果快照见[18:16记录](./P1_OUTPUT8192_PROGRESS_20260912_1816.json)。未把错误文案当作开关未传的证明，未重试或修改冻结r10；独立原始响应日志诊断仍等待用户确认。

MindCube/3.8片2=`70271_2`已自动接替启动，片0/1继续；片0已20/95（答案解析错误2），片1已6/95（答案解析错误0）。MMSI/27B片4已1/100。保存记录仍为8192、thinking=false、固定source指纹，0skip，没有完整分片。片1新产物的两道题已在上一轮验证数据→staging→QA图序一致。

补交MindCube/3.8片4–5为`70281_4`/`70281_5`，数组%1；同组全部数组70207(%1)+70263(%1)+70271(%2)+70281(%1)合计最大并发5，符合账户总5作业限制。当前运行5/5、10×A100：MMSI/27B片4/5，MindCube/3.8片0/1/2；排队MindCube/3.8片3/4/5、MMSI/72B片0，加无关55278后提交10/10。没有取消健康作业，没有重配环境或重扫权重。H20全满，A10011/15各单卡空闲不够一片，且当前账户运行额度已满。全量完成时间仍受失败诊断和实际题速影响，无法可靠估计。

## 历史进展：2026-09-12 17:48 两片新增标签失败，集中补充MindCube/3.8

新增失败：MindCube/9B片5=`70200_5`（1:29:26，17/95）和MMSI/27B片3=`70219_3`（10:56，1/100），均在评分阶段被thinking标签检查拦截，FAILED 1:0；不是新I/O/OOM证据。9B已提交0–5六片现全部失败，MMSI/27B累计0/3两片失败，均保留现场、暂停扩片且不原样重试。错误文案不证明enable_thinking开关未传；上一轮重建请求没有复现标签错误，不能据此放宽检查。详见[进度及失败日志](./P1_OUTPUT8192_PROGRESS_20260912_1747.json)。

已有MMSI/27B片4/5自动接替，当前片1/4/5运行（70214_1、70219_4、70250_5），每片2×A100；MindCube/3.8片0（70207_0）继续，每片2×A100，已17/95。四个服务均有约28.5 token/s的活动生成记录，无完整分片。仍为8192/BF16/no-thinking/65536/冻结r10，0skip，不更改提示词、采样或解析器。

两个空余提交槽补交MindCube/3.8片2–3=`70271_2`、`70271_3`（数组%2），加已有片0及排队片1=`70263_1`，同组跨数组最大并发4，账户仍限制总5运行。MMSI/27B片2=`70214_2`、MMSI/72B片0=`70202_0`继续排队。当前4运行/8×A100、10/10提交（含无关55278）；兼容A100和H20节点GPU实查均满，无法即时启动第5作业。提交前检查目标片没有产物/attempts且未在队列，无重复，无环境重装，无权重重扫。全量ETA因标签失败仍不可可靠估计。

下一步诊断需保存被guard拒绝的**原始完整响应**；现冻结r10是在guard通过后才保存，当前只读日志无法还原。建议独立诊断版本仅补响应日志，不改8192/no-thinking等推理参数、不覆盖正式r10结果；尚未实施，等待用户确认。健康作业和其他P1监控继续。

本轮末次队列更新：MindCube/3.8片1=`70263_1`已在hkbugpusrv15获得2×A100，当前**5/5运行、10×A100、10/10提交**。运行组合为MMSI/27B片1/4/5和MindCube/3.8片0/1；MindCube/3.8片2/3及72B片0受QOSMaxJobsPerUserLimit排队。片1是新获GPU启动状态，不冒称已保存首题。

## 历史进展：2026-09-12 17:33 补交任务与输出重复诊断

本轮先补交MMSI/27B片5为`70250_5`，随后原首片`70201_0`在运行1:09:49后也因评分响应thinking标签检查退出FAILED 1:0，未重试。已有片3=`70219_3`自动接替获得GPU，片1继续。MMSI/27B同类失败需诊断，暂不再扩大该组；保留已运行/已排队片，不修改冻结r10。新空出的提交槽补给已有成功题的MindCube/3.8片1=`70263_1`，与片0合计最大并发2。所有新提交实际export保持8192，复用既有环境及资源校验记录。

| 实验 | 运行分片 / 作业 | 排队分片 / 作业 | 每片GPU |
|---|---|---|---|
| MMSI / Qwen3.5-27B | 1=`70214_1`、3=`70219_3` | 2=`70214_2`、4=`70219_4`、5=`70250_5` | 2×A10080 |
| MMSI / Qwen2.5-VL-72B | 无 | 0=`70202_0` | 3×H20 |
| MindCube / Qwen3.5-9B | 5=`70200_5` | 无；0–4失败保留 | 2×A10080 |
| MindCube / Qwen3.8-27B | 0=`70207_0` | 1=`70263_1` | 2×A10080 |

合计4运行、8×A100、10/10提交槽（含无关55278）。账户限制实查5运行/10提交；兼容11节点仅剩1GPU，12/15/16全满，因此不能用单卡空余启动2/3卡任务。17:28部分题数MC9B片5=15/95、MC3.8片0=12/95、MMSI27B原片0=2/100；MC9B片5答案解析错误1、MC3.8片0答案解析错误1，0skip，未有完整分片。MMSI日志显示约28.5 token/s持续生成，慢不等于未启动；当前质量问题和排队使全量ETA不可靠。

隔离诊断：使用已加载9B服务，将失败片2的step2 rank0内容与step1 rank1系统提示重建为rank1评分请求，seed44/temperature0/top_p1/max_tokens8192/enable_thinking=false，绕开localhost代理后取得原始响应，耗时124.58秒。返回reasoning=null、reasoning_tokens=0、无think标签，但正文反复重复直到completion_tokens=8192、finish_reason=length。**这只证明该重建请求在no-thinking开关下仍发生输出重复/截断，没有复现原标签失败，也不是原失败响应。** 完整诊断响应单独归档于[诊断JSON](./P1_OUTPUT8192_DIAGNOSTIC_20260912_1730.json)，不混入正式计分。未修改提示词、采样参数、解析器或guard，不将开关为false夸大为所有返回均正常。

## 历史进展：2026-09-12 17:12 参数复核与分片2失败

DAAI正式launch_manifest再次确认max_output_tokens=8192、thinking=false、dtype=bfloat16、max_model_len=65536。MindCube/9B分片2也在评分阶段触发thinking标签检查，累计0/1/2/3/4五片失败，不原样重试、不放宽检查。一次利用已有服务的独立诊断请求在发送时遇到Broken pipe，未取得原始响应，不能据此判定模型实际开启thinking；没有修改正式源码或结果。

当前运行：MindCube/9B片5（70200_5）、MindCube/3.8片0（70207_0）、MMSI/27B片0/1（70201_0、70214_1），每片2×A100，共8卡。排队：MMSI/27B片2/3/4（70214_2、70219_3、70219_4）、MMSI/72B片0（70202_0，3×H20）。含无关55278共9个提交任务。本次只读诊断及记录更新，不取消健康作业、不重复提交；旧1024和历史结果保留，未产生新全量成绩。

## 历史进展：2026-09-12 16:30 同类标签失败扩大，正常setting接替

MindCube/9B新增片0、4在评分请求后触发同一thinking标签检查；累计0/1/3/4四片FAILED 1:0，均不原样重试。片2、5仍运行，不取消健康作业。16:28快照片0/1/2/3/4/5分别4/2/15/3/1/3题（每片95题），无完整片。MMSI/27B `70201_0`、MindCube/3.8 `70207_0`已经获得GPU接替启动；MindCube/3.8已保存3题且答案解析错误为0。新进度见[快照](./P1_OUTPUT8192_PROGRESS_20260912_1630.json)，仅为部分题数，不报告全量成绩。

进一步只读排查：本地Qwen3.5-9B聊天模板确有enable_thinking=false分支，追加空think区块作为生成前缀；当前server及request开关写法与[vLLM官方说明](https://docs.vllm.ai/en/latest/features/reasoning_outputs/)一致。评分原响应在检查通过后才打印/落盘，因此被拒绝的完整响应确实缺失；不能仅凭异常文案判断开启了thinking，也不能直接删除标签或放宽检查。尚需独立捕获失败响应以区分模板/解析器与模型重新生成标签，正式冻结r10未改。

暂不扩大有同类错误的MindCube/9B。两个空出的提交槽位补给MMSI/27B片3–4，数组`70219`（%1）；与`70201`和`70214`跨数组合计最大并发3，仍受账户总5运行上限约束。已排MMSI/27B 0–4片，无重复；72B首片继续排队。当前运行4作业、8×A100，提交10/10；兼容11节点只剩单卡、12/15/16无空卡，现有排队任务等待可适配资源补第5位。所有设置/源码/旧产物保留。

## 历史进展：2026-09-12 16:15 8192实际产物通过，但两片被thinking标签检查拦截

MindCube/9B实际结果记录8192、thinking=false、BF16、65536及固定r10源指纹；五个已有产物分片各抽查一题，原图→staging→最终QA图片顺序一致，已保存的抽查回答无thinking标签。当前快照：片0=3/95、片1=2/95、片2=6/95、片3=3/95、片4=1/95；片5刚启动未保存首题。0skip，尚无完整分片。详见[实时核验和失败说明](./P1_OUTPUT8192_LIVE_AUDIT_20260912_1615.json)。

`70200_1`、`70200_3`在评分请求返回后被`_validate_no_thinking`检测到think标签而退出FAILED 1:0，不是I/O/OOM。服务端实际日志明确`default_chat_template_kwargs={'enable_thinking': False}`、parser=qwen3，故错误文案不能直接证明开关未传；失败响应在抛错前未落盘，尚不能区分模型重新输出标签、字面标签误报或模板/parser问题。**两片保留现场，暂不原样重试、不绕过检查、不修改冻结源码。** 健康的0/2/4/5继续，不因该问题取消它们。

补交MMSI/27B分片1–2为`70214`（%1），与`70201_0`合计同组最大并发2；实际export保持8192。已有旧组合成功证据，未重复smoke。当前4个运行作业均MindCube/9B，共8×A100；MMSI/27B 0/1/2、MMSI/72B 0、MindCube/3.8 0均排队，加无关55278后10/10提交槽位已满。兼容11节点只剩1张A100，12/15/16均无空卡，暂不能填第5运行位；资源释放后已排队的其他setting补位。无重复提交，不触碰无关作业。

## 历史进展：2026-09-12 15:58 按用户要求切换8192、保持no-thinking

用户明确要求将最大输出由1024改为8192。该预算同时作用于搜索评分和最终作答，故本次使用独立r10源码、Run ID和结果根目录从头重跑正确四项P1，不只补解析失败题，也不把旧1024题混入新结果。BF16、no-thinking、65536上下文、官方图片顺序、论文搜索参数及发布SVC20步均未改变。8192是用户指定适配参数，不宣称为论文参数。

源码commit `c1a10559a7ca54136283960a629aeda3d8bb6cad`，source SHA256 `27b260c67242f0954373643d2b26ed2861394f0697c9cf7856a6d2dac249ebdf`；冻结工作目录 `/home/comp/24482277/shiyang/MindJourney-p1-output8192-r10-20260912`，新运行根 `/home/datasets/shiyang/daai_runtime/p1_output8192_runs`。不修改旧r9。配置、实际请求、运行脚本、提交export、launch_manifest及结果预算验证一致更新为8192；Qwen3客户端和服务端均保持enable_thinking=false，返回thinking内容时拒绝继续。Qwen2.5-VL没有thinking开关，不向其发送不支持的选项。92项本地测试通过，无远端环境重装/import/version/smoke检查。

| 8192正确P1 | 作业与分片 | GPU请求 | 15:58状态 |
|---|---|---|---|
| MindCube / Qwen3.5-9B | `70200`，0–5，数组并发4 | 每片2×A10080 | 0–3 RUNNING启动中；4–5候补 |
| MMSI / Qwen3.5-27B | `70201_0` | 2×A10080 | 排队 |
| MMSI / Qwen2.5-VL-72B | `70202_0` | 3×H20，TP2+SVC1 | 排队，实际export保留0.93 |
| MindCube / Qwen3.8-27B | `70207_0` | 2×A10080 | 排队 |

当前4片获得8张A100、10/10提交槽位（含无关55278）。策略仍优先用满账户额度，再集中同组；其他已排队组合可填第5运行位。新作业组合/数据/源指纹检查通过；此时尚未核验8192正式首题，不把Slurm RUNNING冒称为推理成功。完整参数、提交stdout、launch_manifest见[8192执行清单](./P1_OUTPUT8192_EXECUTION_20260912.json)。

本次参数切换已停止旧数组70165/70166/70167/70168/70179；Slurm子步骤FAILED 15:0来自主动终止，不是新I/O故障。旧结果全部保留原路径：MindCube/9B三片52/95、45/95、54/95；MindCube/3.8为32/95；MMSI/27B为16/100。旧产物路径及SHA见[迁移保留记录](./P1_OUTPUT8192_MIGRATION_20260912.json)。历史完整6组结果不删除、不修改得分。旧任务的ETA失效，等待8192题速后重新估计。

## 历史进展：2026-09-12 15:46 运行与提交额度均满

当前 5/5 运行作业、10×A100 80GB，10/10 提交任务（含未修改的无关55278）。MindCube/9B 分片2已自动启动，无需新提交；已有候补受 QOSMaxJobsPerUserLimit 限制。已确认兼容的11/12/15节点及H20节点16 GPU均已分配，无额外可用资源。不取消健康作业，不重复提交。

| 实验 | 运行分片 / 作业 | 已答题数（非全量成绩） | 排队 |
|---|---|---:|---|
| MindCube / Qwen3.5-9B | 0=`70167_0`、1=`70179_1`、2=`70179_2` | 48/95、40/95、49/95；合计137/1050 | 3–5=`70179_3`–`70179_5` |
| MindCube / Qwen3.8-27B | 0=`70168_0` | 31/95 | 无新增 |
| MMSI / Qwen3.5-27B | 0=`70165_0` | 16/100 | 无新增 |
| MMSI / Qwen2.5-VL-72B | 无 | 尚未启动 | 0=`70166_0`，3×H20 |

五片结果及日志均近期更新，0 skip，Slurm 无新增失败，日志末尾无 I/O/OOM 错误。尚无完整分片，不提前生成 COMPLETE 或全量结果。固定源指纹、BF16/no-thinking/65536 保持一致。[本轮监控快照](./P1_PROGRESS_20260912_1546.json)。

质量风险：MindCube/3.8 当前31道中14道答案解析失败，按既有协议计错。抽查3条原响应均在分析中途结束、没有最终答案，疑似固定1024输出token上限导致截断；客户端未保存finish_reason，不能将原因认定为已证实。保留原响应，不擅自增大上限、改提示词或解析器，不重启正在运行作业。MindCube/9B三片累计答案解析失败6道；MMSI/27B为1道。

按当前平均进度粗估，运行中的MindCube/9B三片还需约2–3小时；该组若保持3片并行、后续题速相近，剩余全量约需12–16小时。MindCube/3.8当前片约4–5小时，MMSI/27B当前片约11–12小时。仅作排程参考，不含后续资源变化；72B及四组全部完成时间尚无法可靠估计。

## 历史进展：2026-09-12 13:55 首题产物核验并增加同组并行

三组已经实际推理，首片快照：MindCube/9B 5/95、MindCube/3.8-27B 5/95、MMSI/27B 2/100；72B 仍等待 H20。已核对这 12 道完成题目的原始题目、图片 staging 顺序与实际 QA prompt 图片顺序，均一致；运行记录的固定源码、BF16、no-thinking、65536 未变化。详见[实时产物核验摘要](./P1_LIVE_AUDIT_20260912_1355.json)。分片未完成，以上不作为全量成绩。

MindCube/9B 首题成功后补交分片 1–5，数组 `70179`（%3），加上原数组 `70167_0`，同一实验总并发上限 4。`70179_1` 已在 hkbugpusrv12 获得 2×A100，2–5 待资源；其余三组不取消、不重启。账户此时 4 个运行作业、8×A100，10 个运行/排队任务（含无关55278），提交槽位已满；运行上限 5 尚未满，可兼容节点空余 GPU 不足下一组申请。保留 H20 优先的 72B 排队任务。

原执行 JSON 的 submit_args 漏记初始提交继承的缓存环境变量，首次补提在 sbatch 前因默认缓存缺失被拒绝，未生成作业、未改变运行中任务。补齐原始 `P1_CACHE_ROOT` 和 `P1_MODEL_VALIDATION_ROOT` 后成功复用既有缓存，未重新配环境。已将这两个必需变量补入[执行清单](./P1_CORRECTED_EXECUTION_20260912.json)，后续续提必须一并传入。

需继续观察但不擅自改变设置：当前评分解析错误计数 MindCube/9B=6、MMSI/27B=4；MindCube/3.8 的答案解析失败 2 道，按既有协议计错。0 skip，无 I/O/OOM 失败证据。保留原输出，不改 prompt/token 上限以追求分数。全量 ETA 暂不可靠，待更多题耗时和并发稳定后更新。

## 历史进展：2026-09-12 13:36 正确 P1 三片获得 GPU

- `70165_0`（MMSI/27B）及 `70167_0`（MindCube/9B）在 hkbugpusrv11 启动，各 2×A100 80GB。
- `70168_0`（MindCube/3.8-27B）在 hkbugpusrv12 启动，2×A100 80GB。
- `70166_0`（MMSI/72B）仍为 PENDING / Priority，请求 3×H20。

目前合计 3 个运行作业、6 张 A100；已复用节点本地依赖，三个运行作业的组合与输入指纹检查通过。尚未核验首题产物，启动状态不等于已成功推理。

自动监控已恢复为每 10 分钟一次。调度原则严格按用户要求：**先尽量用满账户额度，再集中完成同一组实验**；新组合首题核验后立即并行补片，不等待整片完成。优先集中 MindCube/9B，MMSI/27B 补充利用资源；根据实际空闲 GPU 调整，不为集中运行闲置可用额度。不取消健康作业，不重复提交。

## 历史进展：2026-09-12 13:30 正确 P1 已重新提交

已完成[论文、预处理与最终图片顺序核对](./P1_RESTART_AUDIT_20260912.md)，本地 89 项测试通过。数据全量验证无缺图，题目/答案标签/原图顺序一致；实际 submit 与 worker 加载前均有独立防配反合约。新源码 commit `eafd00c`，source SHA256 `d0eca40528e27676e4c47c6bb68f7198f6195259b83f6e7509f3921b958b53b0`；工作目录 `/home/comp/24482277/shiyang/MindJourney-p1-corrected-r9-20260912` 冻结，不拉取后续文档提交。旧 r8 不修改。

| 正确 P1 组合 | 分片 | 作业 ID | GPU 请求 | 13:30 状态 |
|---|---:|---|---|---|
| MMSI / Qwen3.5-27B | 0 / 共10片 | `70165_0` | 2×A100 80GB | PENDING，Priority |
| MMSI / Qwen2.5-VL-72B-Instruct | 0 / 共10片 | `70166_0` | 3×H20，TP2+SVC1 | PENDING，Priority |
| MindCube / Qwen3.5-9B | 0 / 共11片 | `70167_0` | 2×A100 80GB | PENDING，Priority |
| MindCube / Qwen3.8-27B | 0 / 共11片 | `70168_0` | 2×A100 80GB | PENDING，Priority |

以上已由 Slurm 接收，但尚未开始推理，首题产物核验尚未发生；排队不等于成功运行。72B 实际提交 export 明确为 `P1_GPU_MEMORY_UTILIZATION=0.93`。完整 Run ID、固定数据路径、提交参数与归档目录见[执行清单](./P1_CORRECTED_EXECUTION_20260912.json)及各新目录的 `submission_initial.txt`。账户目前 5 个排队任务（含无关55278），没有取消无关作业。

后续在各新组合有首题成功证据并核验后并行补片，不等首片全量完成；按账户限额优先用满资源，再集中推进少数 setting。不得使用旧 `p1_parallel_dispatch.py`：它只适用于错误组合的旧 r8。续提只能依据新执行清单，保留源指纹与 Run ID；每片完成即验证、归档、更新 RESULTS.md 并推送。

## 历史进展：2026-09-12 11:13 错误组合全部完成并严格合并（非用户 P1）
...
- 原Run/r8、本地环境、BF16/no-thinking/65536/官方图片顺序及论文SVC设置不变。未重启健康任务或重新检查环境，所有结果和日志保留；待最后两片完成后严格核验、合并1000题并保存。

## 历史进展：2026-09-12 06:26 MMSI/3.8 完成7/10片，剩余三片全部运行
...
本段此前“用户已确认上述映射”的记载没有依据，现明确撤回。上表是错误的执行计划；用户真正的 P1 始终是 MMSI 的 27B/72B 和 MindCube 的 9B/3.8-27B。历史运行的模型 ID 与原始结果不改名、不移作其他数据集成绩。
